...
for line in sec[auth_start:auth_end].splitlines():
    if not re.match(r"\|\s*`fathom\.auth\.[a-z_.]+\d*`\s*\|", line):
        continue
    cells = line.split("|")
    if len(cells) < 5:
        continue
...
con = collections.defaultdict(set)
cur = None
NAME2SLUG = {
...
for line in D4.splitlines():
    h = re.match(r"## \d+\. (.+)", line)
    if h:
        cur = NAME2SLUG.get(h.group(1).strip())
        continue
    if not cur:
        continue
...
print(
    f"doc04 published: {sum(len(v) for v in pub.values())}  consumed: {sum(len(v) for v in con.values())}\n"
)
...
for ev in sorted(cat):
    if ev not in published_all and ev not in no_doc04_publisher:
        print(f"[MISSING PUBLISHER] {ev} in catalog, published by nobody in doc04")
        errs += 1
for slug, evs in pub.items():
    for ev in sorted(evs):
        if ev not in cat:
            print(f"[NOT IN CATALOG] {slug} publishes {ev}, absent from doc03 catalog")
            errs += 1
...
for ev, consumers in sorted(cat.items()):
    for c in sorted(consumers):
        if c in NAME2SLUG.values() and ev not in con[c]:
            print(f"[DECLARED NOT CONSUMED] doc03 says {c} consumes {ev}; doc04 does not")
            errs += 1
...
for slug, evs in sorted(con.items()):
    for ev in sorted(evs):
        if ev not in cat:
            print(f"[UNKNOWN EVENT] {slug} consumes {ev}, not in catalog")
            errs += 1
        elif slug not in cat[ev]:
            print(f"[UNDECLARED DEPENDENCY] {slug} consumes {ev}; doc03 does not list it")
            errs += 1
