...
| ID | Sev | Finding | Disposition |
|---|---|---|---|
| **D40** | MED | **`design_change.projected` contaminating operational predictions was an unreviewed path.** Document 04 §10 required the event and required it "strictly segregated" from operational predictions; document 04 §4 listed the segregation mechanism as an open Phase 3 question. Neither review pass produced a finding on it — the risk that a forward-looking design scenario leaks into and biases live PdM predictions survived both adversarial passes undetected, flagged only when Design Advisory's build-framework agent (`docs/build/28`) went to implement the producer side and found no aggregate, no topic, and no consumer obligation actually specified for it | **FIX — partially applied; the "applied" status was overstated, per `42-redesign-case-builder.md` §18 item 5.** Producer-side mechanism specified in `28-design-advisory.md` §7.2 (separately topiced, separately stored); `DesignScenario` added to document 04 §10's aggregate table. **But `docs/build/22-pdm.md` does not reflect DA→PDM-1**: it addresses the hazard through a *different*, independently-adequate mechanism (`serving_class = 'research_only'` under RLS in the same prediction table, not a scenario-scoped store keyed on `scenario_id`), never cites document 28, and leaves three of DA→PDM-1's five clauses unaddressed — no statement that the event is not an invalidation trigger, no `computed_against_baseline_epoch` supersession rule, no forward-looking-surface rule — and doc 22's test list never mentions 28's contributed consumer-driven test. Either doc 22 is amended to state all five clauses explicitly (accepting that `research_only`/RLS satisfies clauses 1–2), or doc 28 §7.4 is amended to accept doc 22's mechanism as satisfying them and add the missing three. Not closed by this entry — recorded here so the disposition is honest rather than reopening D40 as a new finding |

### 2.9 Findings surfaced during Wave 5 (agent-runtime build-framework authoring)

Most Wave 5 corrections were fixed inline across documents 03/09/10/12/22/23/24/28/30/31/32/34/35 as they surfaced (the authority-naming collision, the gateway's superseded two-hop token model, the missing agent-invocation surface, four missing manifests, the generalized dual-control check, and others) and are not re-numbered here. Two require a program decision rather than an editorial fix.

| ID | Sev | Finding | Disposition |
|---|---|---|---|
| **D41** | MED | **The three agent runtimes have silently diverged on Domino-hosting, and nothing traces the divergence.** Document 01 §8's opening and its §3 plane table both assert agent runtimes are Domino-hosted, with §8.7's Sustainment-Plane contingency framed as available but not adopted. Building against that framing, the Maintainer Copilot (`40-copilot.md` §13.1) and the Redesign Case Builder (`42-redesign-case-builder.md` §13.1) each independently adopted the §8.7 contingency outright — four independent grounds apiece, converging on the same conclusion — while the PMA Pre-Screener (`41-pma-prescreener.md` PS-OQ-11) tentatively keeps its enterprise trigger as a Domino Job, marked `[VERIFY]`. Two of three demonstration agents have now departed from document 01's stated default, and document 01 itself has not been told | **DECIDE** — see below |
| **D42** | LOW-MED | **The capacity model (document 06 §7/§6) has no figures for two of the three demonstration agents' own usage, discovered only when their build documents needed a number and found none.** `40-copilot.md` §16 correction 22 (its own **OQ-40-1**): no questions-per-maintainer-per-day, concurrent-session, turns-per-session, tokens-per-turn, or answer-latency figure exists, though replica sizing and the LLM concurrency envelope depend on them. `42-redesign-case-builder.md` §18 item 20 (its own **OD-RCB-4**): document 06 §6's adjudication-effort figures (12 candidates/review, ~45 s/candidate) are anomaly-tag-specific, and no figure exists for adjudicating a `redesign_case` — plainly not a 45-second act, and the proposal kind with the largest consequence if rushed. Both documents correctly declined to invent one (09 §9 item 31) | **DECIDE** — see below |

**D41's decision.** Either document 01 §8/§3 is amended to state that the Sustainment-Plane contingency is adopted for the two interactive/long-running agents specifically (with PMA's enterprise trigger remaining a documented open question pending the Domino-programmatic-invocation verification `41-pma-prescreener.md` PS-OQ-11 names), or a reviewer determines the two build documents' reasoning does not hold and the contingency should not be adopted — in which case both build documents need rework, not just document 01. Recorded here rather than decided unilaterally because it changes document 01's own stated architecture, not merely an implementation detail beneath it.

**D42's decision.** Two candidate paths, neither evaluated here: source both figures the way document 06 §7's existing rows were sourced originally (against whatever process produced those numbers), or accept that both are genuinely unknowable before a pilot and set them as tunable Helm values with a stated review cadence once real usage data exists. Either way, `06-demo-decisions-and-assumptions.md` §7 needs the two new rows, in the confidence-marked form its existing table already uses.
