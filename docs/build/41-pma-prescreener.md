...
**[AMENDMENT — resolved.]** `03-integration-contracts.md` §6 now records the resolution directly beneath the `mission.completed` row: the existing `pma` consumer entry is stated to already cover the run-initiator's bridge, explicitly *"closing `41-pma-prescreener.md`'s PS-OQ-1, rather than adding a tenth platform-service row"* — a program decision, made, not merely proposed. §20 item 1 and PS-OQ-1 (§21) are closed to match; this document had not been told the decision landed.
...
| Early completion | **[AMENDMENT — corrected; previously described three incompatible ways across this section, §2.4 step 16, and §14.1.]** The window closes early on receipt of **`agent_run.completed`** (`fathom.auth.agent_run.v1`, 31 §11.1), which `pma` consumes filtered to `agent_id='pma-prescreener'` and correlated to the waiting review via `mission_id` in the event's `subjects` (§2.4 step 3 puts it in the grant scope for exactly this reason). This is the one mechanism that closes the window whether the run produced proposals or not — a run cannot complete without `auth` recording it, so **the zero-proposal case (§7.1's first-class success) closes the window exactly as reliably as a proposal-bearing one**, unlike a signal riding the last `POST /proposals` call, which a zero-proposal run never makes and which would otherwise burn the full window every time. Waiting the full window after the agent has finished is wasted latency on the critical path |
...
| 3 | Grant requested: `POST /auth/autonomous-grants` with the accountable owner and a scope of exactly this asset **and this `mission_id`** [**AMENDMENT**] — `mission_id` is in the grant scope specifically so `agent_run.completed`'s `subjects` (31 §11.1) carries it, which is what lets PMA correlate the run's completion to this review without a direct call (§2.3) | run-initiator | `422 accountable-owner-required` or a disabled owner ⇒ **no run**, audit record, alarm. There is no fallback identity (31 §3.3 rule 1) |
...
| 16 | Run completion recorded (`POST /agent-runs/{run_id}` terminal state, per 31 §4.4) — `auth` emits `agent_run.completed`; `pma` consumes it and closes the quiesce window early | runtime → `auth` ──event──▶ `pma` | — |
...
**[AMENDMENT — corrected from "exactly six."]** The run has exactly **seven** terminal outcomes; a row for `terminated_revoked` was missing despite `31-auth.md` §4.3's `agent_runs.status` enum already contemplating it, and G8 asserting only six. Each is recorded to Audit with the accountable owner (03 §8.3) and to the run record (31 §4.3). None is a silent success.
...
| `terminated_revoked` | **[AMENDMENT]** The accountable owner's grant is revoked mid-run (`31-auth.md` §4.3's `agent_runs.status` already names this value; no row here ever used it) | Terminate immediately on the next authenticated call's `401`/`403` once the revocation takes effect. **No proposal is created after the revocation**, same rule as an authority lapse — a revoked grant is not a live one regardless of the token's own unexpired `exp` |
...
Reused as-is. 20 §6.1 makes every read operation `x-agent-eligible`, and 20 §6.1 names the manifest (`registry-configuration-lookup.v1`, in `packages/agent-tooling/manifests/registry/`). The operations this agent calls:
...
| **Ship state** | **Disabled.** `tool-pins.yaml` carries it; the grant omits `document_chunk` from `declared_scope.aggregates` until it is enabled. **[AMENDMENT — noted, not restructured.]** This is a runtime-layer disablement (an OPA refusal on an unexpanded scope), not a structural absence (the manifest stays pinned as one of the five, so PS-T10's "exactly five manifests" count does not move) — deliberately, the same pattern `33-notification.md` uses for its own disabled-until-catalog-amendment handlers: *"ship complete, tested, and disabled by Helm value"* rather than absent from the build. Enabling it once D38 lands is then a `declared_scope` change, not a manifest addition or a new conformance test |
...
| Tool calls | Through `tool-server` → `gateway` (§4.9) | **None.** **[AMENDMENT — corrected.]** Previously described as *"in-process reads against the edge PMA and edge Telemetry instances"* — accurate for PMA's own state (this mode ships inside PMA's edge release, same process, no edge to sanction), but a direct in-process read against a *different* sub-application's edge instance (Telemetry) is exactly what 09 §4.4.2's edge table and §17 item 31 forbid: *"[d]o not call a sub-application directly … `runtime → tool-server → gateway → target`."* This mode is not a runtime with that path available, but the fix is not to bypass the rule — it is that PMA already has a sanctioned edge for exactly this producer: `23-pma.md`'s own `CONSUMES` already declares `telemetry` as a producer (`anomaly.detected`, `mission.completed`), which 09 §4.4.2's *"any sub-application → its own declared producers, `changed_since` reads only"* row already sanctions. Reading Telemetry's edge detector output is that same declared-producer relationship, GET-only, evaluated at the edge tier rather than enterprise — not a new edge, and not an in-process shortcut around one |
...
| **G8 — Refusal behaviour** | **[AMENDMENT]** The seven run outcomes of §2.6 each occur under their trigger, with no proposal after an authority lapse or revocation, and no retry after a 429 | Yes |
...
| `ps-budget-under-ceiling` | **[AMENDMENT]** Two conditions, not one — the bare `< 20` ceiling check alone passes a value (8, 92% of ceiling) §6.2's own reasoning rejects for leaving "no headroom for two other agents," so a future config bump to 8 would pass a gate that document's own prose says should fail: (a) `proposal_budget_per_mission × (missions_per_month / 30.4) < 20`, using 06 §6's mission count and 06 §7's proposal ceiling; **and (b) `proposal_budget_per_mission <= 23-pma.md §3.4's per-equipment-family cap`** (6, `ceil(candidate_cap / 2)`) — the non-invented figure §6.2 actually derived the adopted value from, which is what bounds one run's worst-case share of a single family and, incidentally, keeps daily throughput well under the raw ceiling |
...
| Result | Polled. The initiator records the run's terminal outcome; PMA learns of completion through `agent_run.completed` (31 §11.1, §2.3), never by calling the agent |
...
| **Domino runtime → program services** | `domino-compute → tool-server`, sanctioned **[AMENDMENT — this row previously cited `domino-compute → gateway`, an edge this workload does not use at all: the run-initiator's own first hop is to `tool-server`, not `gateway`, and 09 §4.4.2 had no `domino-compute → tool-server` row until this pass added one for exactly this workload]** | 09 §4.4.2's `domino-compute → tool-server` row, added because this Domino Job-hosted runtime carries no `fathom.navy/service` pod label and so cannot use the existing `agents/* → tool-server` row, which is pod-label-selected. Tool calls route runtime → `tool-server` → `gateway` (34 §5.1), and `tool-server → gateway` is a separate, already-existing row this runtime never touches directly |
...
Found while reconciling. Each is a **defect or gap in the cited document**, not a decision of this one. Following the convention 26 §13 uses. **[AMENDMENT]** Items 1, 3, and 4 were part of the four marked here as blocking; all three are now resolved (see their own rows below). Item 2 (21 §9.5 manifest v2) remains open.

| # | Document | Defect | Correction | Status |
|---|---|---|---|---|
| 1 | **03 §6, Telemetry rows** | ~~`mission.completed` lists consumers `pma`, `failure-intel`, `audit`... the bridge C19 promises does not exist~~ **[RESOLVED.]** `03-integration-contracts.md` §6 now states explicitly that the existing `pma` consumer entry covers the run-initiator's bridge (a non-agent component shipped from `agents/pma-prescreener/`, per §2.2), rather than adding a tenth platform-service row | Closed — no correction needed | **Resolved.** Interim in §2.2 no longer applies. **PS-OQ-1 closed** |
...
| 3 | **23 §7.1 and DO-NOT-10** | ~~States that *"the detector ensemble / pre-screener… is **`telemetry`'s**, not PMA's"*, against 11 §1.2, 04 §8, and 21 §7~~ **[RESOLVED.]** 23 §7.1's table now correctly places the detector ensemble with `telemetry` and the small edge pre-screener with PMA itself | Correct 23 §7.1's table row and DO-NOT-10 to place the reduced afloat pre-screener with PMA, retaining the detector ensemble with `telemetry` | Closed — no correction needed | **Resolved.** §9.1 adopts the corrected reading |
| 4 | **23 §2.8, §3.7, §8.5** | ~~An agent-originated candidate is adjudicable through **two** independent surfaces... If both are live and independent, one candidate costs two human acts and 06 §6's 10.5 reviewer-hours are double-counted~~ **[RESOLVED.]** `23-pma.md` now states the rule once: the reviewer's confirm/reject on an admitted candidate *is* the adjudication of its proposal, in the same transaction; `POST /proposals/{id}/adjudicate` is reachable only for a never-admitted proposal (`409` otherwise); the gateway queue renders an admitted agent proposal as `claimed_by_review` | Closed — no correction needed | **Resolved.** No longer PS→PMA-6; §7.6's interim is superseded |
...
| 6 | **31 §4.3** | ~~`agent_runs.status` is `running \| completed \| terminated_authority_lapsed \| terminated_pod_restart \| terminated_revoked`. There is no value for a run stopped by a **target refusal**~~ **[RESOLVED.]** 31 §4.3 now includes `terminated_target_refused` in the enum | Closed — no correction needed | **Resolved.** No longer PS→AUTH-1; §2.6's interim is superseded |
...
|---|---|---|---|---|
| **PS-OQ-1** | ~~**Who is the catalogued bridging consumer of `mission.completed`?**~~ **[CLOSED.]** `03-integration-contracts.md` §6: the existing `pma` consumer entry covers it | The catalog-parity gate (09 §8.2), and the consumer-driven conformance test for the trigger | The `run-initiator` shipped from `agents/pma-prescreener/`, with full inbox semantics and no domain logic (§2.2) | Resolved |
