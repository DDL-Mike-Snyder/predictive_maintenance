...
**A note for implementers reading document 10.** 10 §4.7 types `authority_class` as an opaque string and records **OQ-13** — "the vocabulary of `authority_class` is undefined" — as "the most consequential gap in the package," blocking D16's authority check. **03 §7.2.1 closes OQ-13.** PMA validates `authority_class` against the `AuthorityClass` vocabulary (`maintainer | planner | supply_officer | design_authority | fleet_authority`) and asserts `maintainer` for every proposal it owns. The shared package should be narrowed from `NonEmptyStr` to a `StrEnum` in its next minor version; until it is, PMA's own validation is the enforcement point, and §13 records the item.
...
| `POST /proposals/{id}/adjudicate` | Approve/reject with re-validation (§2.8); `If-Match` required | `required` | `state-changing` | no |
...
|---|---|---|
| **1. The detector ensemble and the pre-screener keep running** | PMA does nothing to them | They are `telemetry`'s (04 §3, 11 §1.2), and at the edge they are the ship's only candidate source. PMA has no authority over a sibling sub-application's workload, and 03 principle 3 forbids emitting an event to instruct one. A design in which PMA could silence a sibling's detector would be a cross-domain command dressed as an event |
...
| **5. `POST /proposals` is refused** | `429` with `urn:fathom:problem:pma:admission-control-engaged` and `Retry-After` | Proposals arrive over HTTP and can be refused with a retryable status; events cannot. Refusing them halts the enterprise pre-screener's contribution and, per 03 §8.3, gives its accountable owner a signal. The agent terminates with a resumable checkpoint rather than continuing; it does not create proposals after the refusal |
...
| **The detector ensemble / pre-screener** | Yes — but it is **`telemetry`'s**, not PMA's | 11 §1.2 places the "edge-resident detector ensemble producing anomaly candidates `[D18]`" and the small pre-screener under `telemetry`. PMA consumes `anomaly.detected` with `origin: edge` and does not host a detector |

That last row is the D18 boundary and is the single most misread line in this design. 01 §12 and 06 §4 put the detector ensemble and a reduced pre-screener afloat; 11 §1.2 assigns them to `telemetry`. PMA's afloat obligation is to have a **working review workflow that consumes whatever candidates arrive**, including none — a review with an empty eligible set is created, recorded, and closed as such, never presented as an authoring surface (04 §8).
