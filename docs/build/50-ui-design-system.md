...
- [ ] Corrections **1–18** of §13 each filed against their document with an owner. **[AMENDMENT]** Of 1, 2, 3, 7, and 10, only **3** remains open; 1, 2, 7, and 10 are resolved
...
Found while reconciling, following [09 §11](09-monorepo-and-conventions.md)'s convention: each is a **defect in the cited document or in the approved wireframe**, not a decision of this one. **[AMENDMENT]** Of corrections 1, 2, 3, and 7 (originally marked blocking) and 10, only **3** remains open (its own interim is implemented, §3.5); **1, 2, 7, and 10** are all resolved — see their own rows below.

| # | Document | Defect | Correction | Status |
|---|---|---|---|---|
| 1 | **09 §3.1, §3.2, §2.6** | ~~The monorepo tree, the per-directory governance table, and the pnpm workspace scope have no place for a shared UI package~~ **[RESOLVED.]** `09-monorepo-and-conventions.md` §3.1's tree now includes `packages/ui/`, §3.2 governs it to this document `[amendment 50-1]`, and it is in the pnpm workspace scope | Closed — no correction needed | **Resolved.** §2.1 |
| 2 | **27 §7, and the [06 §2] metric generally** | ~~**No service, no build document, and no operation owns or serves** warning lead-time coverage~~ **[RESOLVED.]** `32-audit.md` §10.7 adds `GET /effectiveness/warning-lead-time-coverage?scope=&id=&horizon_days=&stratum=`, and `27-fleet-status.md` §8's OD-7 marks this resolved | Closed — no correction needed | **Resolved.** Sheets 01 and 01B have an operation to call; [27 §7.5]'s presentation rules (distribution, denominator, chance reference, achievable ceiling, never viewer-filtered) bind the response |
...
| 7 | **30 §8.1 and 30 OQ-9 versus 31 §4.1** | ~~No operation returns the session's identity... no logout mechanism appears anywhere~~ **[RESOLVED.]** `30-gateway.md` §8.1.2 adds `GET /api/v1/gateway/session` and `POST /api/v1/gateway/session/logout`; OQ-9 is corrected to read roles from the session identity rather than a token the console never holds | Closed — no correction needed | **Resolved.** §9.2's speculative shape is superseded by the landed operation's real shape (also corrected this pass — see §9.2) |
