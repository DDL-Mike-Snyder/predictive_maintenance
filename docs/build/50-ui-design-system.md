...
**Interim position, `[ESTABLISHED HERE]`, and flagged as an amendment ask rather than presented as settled** (§13 correction 10, §14 UI-OQ-4):

The practitioner SPA holds **no token either**, exactly like `apps/web`. Its co-resident FastAPI process (§6.2) is the credential holder: it authenticates with a **workload identity** and calls the gateway over the sanctioned `domino-compute → gateway` edge [09 §4.4.2, 31 §5.2], attaching the caller's authority in the second header [31 §5.4] establishes for the Endpoint proxy — `Authorization: Bearer <the calling service's workload token>` plus a caller-authority header — and never accepting a claimed subject from the browser [31 §13 item 15: *"[d]o not trust a caller's assertion of who its end user is"*]. **This reuses [31 §5.4]'s shape for an app→gateway hop it was not written for, which is precisely why it is filed as an amendment ask against [31 §14]'s mechanism rather than asserted here.** Until it lands, `apps/practitioner` is buildable as a read surface against operations its workload identity may reach, and its adjudication actions are blocked — which is the honest state, not a workaround.

The one thing that does **not** differ: **both apps hold a cookie and no token.** That is what lets `packages/ui` be shared without an auth abstraction inside it.
...
**Resolution, `[ESTABLISHED HERE]` and filed as an amendment ask rather than asserted as existing:** one operation is required —
...
Found while reconciling, following [09 §11](09-monorepo-and-conventions.md)'s convention: each is a **defect in the cited document or in the approved wireframe**, not a decision of this one. **None is applied here.** Corrections 1, 2, 3, 7, and 10 block a complete `apps/web`.
...
| 10 | **31 §2.2, §5, §8** | `apps/practitioner` has **no client ID, no token-acquisition path, and no specified credential** for calling the gateway. [31 §2.2] gives a Domino-hosted app a *"Domino session cookie"* traceable to a FATHOM `sub` by brokering, and then warns that federation *"does not put caller identity on a Domino Endpoint invocation … [t]hat gap … must not be reasoned away by pointing at federation."* [28 §2] and [42 §13.3] both place practitioner surfaces there regardless | Extend [31 §5.4]'s two-credential shape to an app → gateway hop, or define a practitioner client and its grant. §6.3 adopts the first as the interim, **which is why it is filed here rather than asserted** | **Blocking `apps/practitioner`'s write path.** Interim in §6.3; flagged |
...
| **UI-OQ-4** | **The practitioner credential path** (§6.3, §13 correction 10) | `apps/practitioner` cannot adjudicate — which is most of what sheets 08 and 09 exist to do | Read-only against operations the app's workload identity may reach; [31 §5.4]'s two-credential shape proposed for extension |
