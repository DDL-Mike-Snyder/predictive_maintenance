...
**[AMENDMENT — real security defect, found in adversarial review and closed here.]** `classification`'s union-label intent was declared (`inherited_from` populated per `03 §7.3`) but no actual population logic was specified to back it, and `affected_population`'s per-hull breakdown is the same D13 shape found and fixed elsewhere in this corpus: a hull's presence in the breakdown discloses its existence to any reader of the dossier, and `blast_radius`'s gate (below) is derived directly from this same breakdown. Because a published dossier is an **immutable, versioned snapshot cited by a business case** (§3.3 above), the fix cannot reproject `affected_population` per reader the way `27-fleet-status.md` renormalizes a live rollup — the same immutability constraint `25-failure-intelligence.md` §2.8's `treatment_census` fix (this same review) had to respect. Fixed the same way: `classification.inherited_from` is populated as the true union of every contributing installed item's own classification (so the dossier as a whole is correctly labelled, closing the union-label half), and `affected_population`'s per-hull breakdown array is **field-suppressed**, not reprojected, for a reader whose clearance does not dominate `classification` — such a reader receives the aggregate counts (`item_count`, `blast_radius`) but not the per-hull line items, mirroring `35-knowledge-retrieval.md`'s DO-NOT-14 pattern. `blast_radius` itself is computed once, at assembly time, over the true full population (so the classification decision it feeds — `AuthorityClass.DESIGN_AUTHORITY` at `class`/`fleet` scope, §8 above — is never weakened by what a later reader happens to be cleared for); only the per-hull breakdown supporting evidence is suppressed on read. Gate G5 (§5.3) — `|{ f ∈ d.field_failures : f.failure_indicator }| ≥ FIELD_FAILURE_FLOOR` — is the same shape one level over: it is evaluated once, internally, over the true full field-failure population sourced from `maintenance_action.recorded`/`installed_item.removed`, for the same reason `blast_radius` is; the individual `dossier_field_failure` rows a reader could enumerate from `d.field_failures` are field-suppressed on read using the identical rule as `affected_population`'s per-hull breakdown, never re-evaluated per reader.
...
**[AMENDMENT — closes §16 correction #4 below; resolved, not merely interim.]** `docs/build/11-outbox-sync-library.md` §9 introduces `installed_item.identity_resolved` on `fathom.registry.installed_item.v1` and names `design-advisory` among its consumers. Document 03 §6's catalog now carries the row. **This service subscribes to it**, extending the existing `installed_item.removed` handler rather than the interim `changed_since`-only reconciliation previously described here: on `resolution: superseded`, every dossier field-failure row carrying the provisional `installed_item_id` is re-keyed to the `canonical_id`, so a dossier cites the item a later assembly would also cite, rather than depending on the next `changed_since` sweep to catch up.
...
| **4** | **03 §6 vs 11 §9** | ~~Doc 11 introduces `installed_item.identity_resolved` and names `design-advisory` among its consumers; the event is absent from 03 §6's catalog~~ **[RESOLVED.]** 03 §6 now carries the row | Closed — no correction needed | **Resolved.** This service now subscribes (§10.2 above) |
...
- [ ] `installed_item.identity_resolved` is subscribed and re-keys dossier field-failure rows on `resolution: superseded`. *(§10.2, §16 #4 — resolved)*
