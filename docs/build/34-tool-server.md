...
1. **Before proxying**, the service writes an `attempted` record: `agent_id`, `agent_version`, `agent_authority` **[AMENDMENT]** (this service's own §8.3 class — `delegated` | `accountable_autonomous`, per §4.5's naming rule; not §7.2.1's `authority_class`, which this service has no opinion about), `accountable_owner` where applicable, `trace_ref`, `X-Correlation-Id`, `bundle_digest`, the tool `name`, `x_fathom_manifest` (name/version/owner), `x_fathom_target` (slug/api_major), the resolved operation, the live spec `ETag` and its measured age, the full arguments, and the outcome of every gate. If `audit` does not accept it, the call is rejected `503` `audit-record-incomplete` and **the target is never contacted.**
...
The interaction that tempts otherwise is `pdm-whatif`, whose interactive tier-3 analysis is Endpoint-backed per 01 §7. But document 01 §7 also fixes the boundary: predictions are read *from PdM, never from Domino*, and `pdm_what_if` is a PdM operation carrying `x-side-effects: none`. **[AMENDMENT — corrected.]** This section previously stated *"[t]he Endpoint hop is PdM's private business, inside PdM's own trust boundary, and PdM attaches caller identity to the audit record on that hop"* — the exact topology `31-auth.md` DO-NOT 13 forbids in terms (*"Do not call a Domino Endpoint from a sub-application directly... Route through the gateway proxy"*), and which `22-pdm.md` §13.1 has now been corrected to match: PdM holds no Endpoint credential and calls `POST /api/v1/gateway/inference/{domino_endpoint_name}` (`30-gateway.md` §5.6), which attaches caller identity, not PdM itself. Every descriptor's operation mapping therefore resolves to a sub-application REST operation, always. A descriptor mapping to a Domino Endpoint URL is malformed and the bundle compiler rejects it.
...
Ingress: `gateway` **[AMENDMENT — was stated as the only ingress; this omitted the agent-runtime ingress 09 §4.4.2 has always sanctioned for this exact service, its whole reason for existing]**, plus `agents/*` (Helm-deployed runtime charts, pod-labelled `fathom.navy/service`, per 09 §4.4.2's `agents/* → tool-server` row) and the `domino-compute` namespace (a Domino Job-hosted runtime — `41-pma-prescreener.md` §14.1's vehicle — carries no such pod label, so this is a namespace-selector rule, per 09 §4.4.2's new `domino-compute → tool-server` row), plus the Prometheus scrape. **No database peer. No Redpanda peer.** The helm-unittest assertion that the rendered egress set *equals* the declared set (09 §4.2, §8.6) is therefore also the assertion that this service reaches no broker and no database — a stronger statement than usual, and worth making deliberately.
...
| Call a Domino Endpoint | Every descriptor maps to a sub-application REST operation. **[AMENDMENT]** The Endpoint hop is inside the **gateway**, not the owning sub-application — the gateway's proxy attaches caller identity there (03 §8.3; 02 §4.3; 31-auth.md DO-NOT 13) | §5.5; the compiler rejects a non-REST operation mapping |
...
| **OQ-7** | ~~**The pin file's name and shape.**~~ **[CLOSED.]** `42-redesign-case-builder.md` §10.5 settles it: `agents/<name>/tool-pins.yaml`, with `bindings[]` for the multi-binding case (34 §2.2's B4 rule checks each) | `agents/<name>/tool-pins.yaml` as in §2.2. Nothing here depends on the filename — only on the compiled binding — so a rename costs a compiler constant |
