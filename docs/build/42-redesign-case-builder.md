...
**[RESOLVED.]** The three documents flagged in §18 correction 7 for carrying pre-A-2 forms — 34 §2.2 (`agent_authority`, snake_case `accountable_autonomous`), 32 §4.3 (`agent_authority` column, snake_case), and 30 §5.3 (`fathom.agent.authority`, not the colon form) — all now use 31's forms. 31 §16.1's Definition of Done is absolute — *"no field named `authority_class` carries an agent class anywhere in the codebase"* — and this runtime and all three documents agree on it.
...
| **16** | **03 §7.2; 10 §4.7** | Three related defects in the shared proposal contract, of which (b) and (c) are **[RESOLVED]**. (a) *"any kind with external legal effect"* is **never enumerated** (10 OQ-12, 26 OQ-S3), and 04 §10's characterization of redesign makes `redesign_case` a live candidate — still open. ~~(b) 10 §4.7's `ProposalKind` StrEnum has six members and omits `purge` and `rewrap`~~ — 10 §4.7 now has all eight members including `PURGE`/`REWRAP` (amendments `[03-1, 03-2]`). ~~(c) 10 §4.7's `authority_class` is still `NonEmptyStr` (OQ-13); 31 §2.4's `AuthorityClass` enum closes it with five values and omits `security_officer`~~ — 10 §4.6b/§4.7 now retypes the field to the real six-value `AuthorityClass` enum, `security_officer` included, closing OQ-13; 31 §13.2 item 12 already reads *"six, as of amendment 03-1"* | (a) Enumerate the set in 03 §7.2. (b), (c) Closed — no correction needed | (a) **Not applied; flagged**, OD-RCB-5; this document adopts 28 §6.4 unchanged. (b), (c) **Resolved** |
