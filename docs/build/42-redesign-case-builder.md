...
- **The token is forwarded unchanged.** 31 §4.1 step 6: *"**THE TOKEN IS FORWARDED UNCHANGED.** The gateway never swaps it for its own workload identity."* 30 §5.3 describes a different, two-hop shape with an `X-Fathom-Delegation` header and a `may_act` constraint; the two documents disagree and §18 correction 7 records it. **This runtime implements 31's shape** — one credential in `Authorization`, forwarded — because 31 is the authority over issuance and delegation lifecycle (31 §4.1) and because a runtime holding two credentials is a runtime that can present the wrong one.
...
**No peer to `design-advisory`, `failure-intel`, `reference-data`, `knowledge-retrieval`, `pdm`, `maintenance`, `registry`, `supply`, or `fleet-status`.** Every one is reached through the tool server, which reaches them through the gateway's pass-through. 09 §4.4.2 sanctions no other edge and none is requested. §18 correction 9 asks 09 §4.4.2 for the missing `agents/* → tool-server`, `agents/* → auth`, `agents/* → audit`, and `agents/* → domino-*` rows, since the table has **no `agents/*` row at all** — which per 09 §9 item 30 means this chart cannot deploy without a doc-09 change plus an ADR. **That is a deployment blocker and is recorded as one.**
...
- It renders the `QualificationReport`, the `CaseDraftPackage` with `suggested_stance` marked as suggested, the derived gaps and limitations in full, and the divergence between the agent's derived lists and the case's committed fields (§6.2).
- It is where a human commits `POST /redesign-cases/{id}/assemble` — the state-changing act the agent cannot perform (§1.3).
...
| **7** | **34 §2.2 (B4); 32 §4.3; 30 §5.3** | **Agent-authority naming drift.** 31 §2.5 (amendment A-2) fixes the claim as `fathom.agent.authority` with snake_case values, and 31 §16.1 requires that *"no field named `authority_class` carries an agent class anywhere in the codebase."* But 34 §2.2's rule B4 reads *"`authority_class` is exactly one of `delegated` or `accountable-autonomous`"*; 32 §4.3's `tool_invocation.authority_class` column comments the hyphenated form; and 30 §5.3's claim names are colon-form (`fathom:authority_class`, `fathom:manifest`, `fathom:agent_version`, `fathom:accountable_owner`) | Rename to `fathom.agent.authority` and `accountable_autonomous` in all three; rename 32's column to `agent_authority`; adopt 31 §3.2's dotted claim names in 30 | **Not applied; flagged.** This runtime uses 31's forms (§7.1). Three documents need the edit and one of them is a database column |
...
| **9** | **09 §4.4.2; 30 §5.3, §8.1** | **Two joined gaps that together block deployment.** (a) 09 §4.4.2's sanctioned-edge table has **no `agents/*` row at all**, so this runtime's egress to `tool-server`, `auth`, `audit`, and the Domino inference path is unsanctioned — and 09 §9 item 30 makes an unsanctioned peer a violation requiring *"an ADR and a change to this document."* (b) 30 §5.3 hop 1 describes *"The operator asks the gateway to invoke an agent"* and doc 30 declares **no route, verb, status code, or polling resource** for it; 30 §8.1 states *"There is no separate 'agent API'"*, which is true of tool calls and leaves invocation undeclared. 30 §14 item 3 already records the related NetworkPolicy gap as *"Blocking deployment. ADR required"* | (a) Add `agents/* → tool-server`, `agents/* → auth`, `agents/* → audit`, and a config-gated `agents/* → domino-*` inference rule to 09 §4.4.2. (b) Declare the invocation contract in 30, or accept §13.4's dispatcher surface as the contract and record it in 09 §4.4.2 as `gateway → agents/*` | **Not applied; flagged. Blocks deployment.** ADR required per 09 §7.5 and §9 item 30. §13.4 specifies an interim surface designed to be superseded |
...
| **14** | **30 §5.3 vs 31 §4.1** | **The delegated-token flow is specified two incompatible ways.** 31 §4.1 step 6: *"**THE TOKEN IS FORWARDED UNCHANGED.** The gateway never swaps it for its own workload identity"* — one credential, one exchange, forwarded. 30 §5.3 specifies **two hops**, the agent presenting **two** credentials (`Authorization` plus `X-Fathom-Delegation`), a `may_act` constraint, and the gateway forwarding *"**only** the hop-2 token"* | Reconcile. 31 is the authority over issuance and delegation lifecycle (31 §4.1), so 30 §5.3 should be brought to 31's shape — or 31 §4.1 amended and 31 §16's DoD updated | **Not applied; flagged.** This runtime implements 31's shape (§7.6): one credential, forwarded. A runtime holding two credentials is a runtime that can present the wrong one |
...
| **17** | **31 §6.4** | **The second adjudicator's authority class is unchecked for `redesign_case`.** The Rego rules `authority_class_insufficient`, `authority_class_not_applicable_at_scope`, and `authority_class_field_stale` all guard on `input.action == "adjudicate"`. The only class check on `second_adjudicate` is `second_adjudicator_authority_insufficient`, scoped `input.resource.kind == "interval_change"`. As written, **a class- or fleet-scoped `redesign_case` second signature need only be a different `sub`, not a second `design_authority`** — so the dual control that 03 §7.2 rule 4 makes mandatory at those scopes can be satisfied by any authenticated human. 31 §16.4's checklist line also names only `interval_change` | Generalize `second_adjudicator_authority_insufficient` to every kind, evaluating the same `any_of` allow-set the first signature is evaluated against | **Not applied; flagged. This is a safety defect on this agent's proposal kind and is the most consequential item in this table after #1.** Nothing in this runtime can compensate: the agent cannot adjudicate, so it cannot detect or refuse an under-authorized second signature |
...
| §8.6 Deployment and boundary | **All items.** | Items 4, 5, 6, 7, 9, 10 bind any container; 1–3, 8, 11 bind the dispatcher and its chart. Item 3 is the one currently unsatisfiable — 09 §4.4.2 has no `agents/*` row (§18 correction 9) |
