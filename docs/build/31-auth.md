...
`POST /api/v1/auth/practitioner-exchange` — a new, narrow token-exchange operation, owned by `auth` and reached through `gateway`'s pass-through surface (`30-gateway.md` §8.1, `[AMENDMENT]`). **[AMENDMENT — corrected; this paragraph previously named `domino-compute → gateway` as the edge while claiming `auth`, not `gateway`, was the target — an internal contradiction, since that edge terminates at `gateway`, and `09 §4.4.2` sanctions no `domino-compute → auth` edge at all.]** The practitioner app's co-resident single-container FastAPI process (`50-ui-design-system.md` §6.2's `02 §4.1`-mandated single-container shape) presents its own client-credentials **workload identity** plus the verified Domino identity JWT (the Domino-session-linked `fathom` `sub` from §2.2's broker-linked record) to `gateway`, over the sanctioned `domino-compute → gateway` NetworkPolicy edge (09 §4.4.2) — the only edge the practitioner host has. `gateway` proxies the call to `auth` verbatim over its own already-sanctioned `gateway → auth` egress (part of the universal "any service → auth" grant, 09 §4.4.2), the identical pass-through discipline every other gateway-fronted operation follows (`30-gateway.md` §8.1/§8.2) — `gateway` does not interpret or validate the exchange, it only proxies. `auth` returns a short-lived, ordinary `fathom`-realm **delegated** access token (§3.2's exact shape, `sub` = the linked human, `aud` = the practitioner surface's own manifest targets), reusing §4.2's RFC 8693 exchange machinery with the Domino identity JWT as `subject_token` and the host's workload token as `actor_token`.
...
| Metrics | `fathom_authz_decisions_total{service,operation_id,allow}`, `fathom_authz_decision_duration_seconds{service}`, `fathom_authz_bundle_revision_info{service,revision}`, `fathom_authz_denials_total{service,reason}` — added to document 09 §5.6's fixed list. **[AMENDMENT]** `reason` is a closed, coarse vocabulary of decision categories (e.g. `time_uncertain`, `insufficient_clearance`, `counter_signature_required_but_not_declared`) — never a resource, principal, or compartment identifier. No label here may vary with which specific resource or compartment was probed, the same discipline `35-knowledge-retrieval.md` §10.1's `KR-CLS-08` enforces by source-level test; a future `reason` value naming a specific compartment would reintroduce exactly the metrics-based existence oracle that document exists to prevent |
