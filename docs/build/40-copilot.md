...
| **G3** | ~~**The delegated-token wire contract is stated three incompatible ways.** 31 §3.2, 34 §4.5/§5.3, and 30 §5.3 disagreed~~ **[RESOLVED — closed by §16 correction 1.]** `30-gateway.md` §5.3 is reconciled to 31 §3.2/34 §4.5's one-token model; all three documents now agree | §16 correction 1 | **Resolved.** §3.3's account of why this document was built against 31 rather than 30 is retained as historical rationale, not a live contradiction |
...
**Why this model and not 30 §5.3's original scheme.** **[AMENDMENT]** 30 §5.3 has since been reconciled to this same one-token model (§16 correction 1); the paragraph below records, historically, why this document was built against 31 rather than 30's original two-hop scheme — two credentials to the gateway (`Authorization` = the agent's own workload token, `X-Fathom-Delegation` = the hop-1 delegation), a `may_act` constraint, flat `fathom:authority_class` / `fathom:manifest` claims rather than 31's nested `fathom.agent` block, and scope strings of the form `pdm.predictions.read` rather than 31's `sfx:` classes. The three reasons for building against 31:
...
Found while reconciling. Each is a **defect in the cited document**, not a decision of this one, following 09 §11's and 26 §13's convention. **This document edits nothing upstream.** **[AMENDMENT — resolved.]** Items 1, 6, 15, 16, 18, and 19 were the ones marked blocking here; all six are now closed (see their own rows below), each with an explicit `[amendment ... closes ...]` back-reference in its target document.
...
| 10 | **32 (Audit) §4** | ~~32's record types cover `tool_invocation`, `agent_run`, `event_ingest`, `prediction_recorded`, `agent_promotion`, and the proposal and adjudication records. There is no record type for an agent's terminal output~~ **[RESOLVED.]** `32-audit.md` §4.3a now defines `agent_answer`, carrying exactly the fields this row asked for, retention class `accreditation` | Closed — no correction needed | **Resolved.** §9.4's last row and §13.4's readiness check now have a real dependency to check against |
| 11 | **34 §2.2 vs 31 §2.5** | ~~34 §2.2's rule B4 spells the second agent class `accountable-autonomous` (hyphen)... 32 §4.3's `tool_invocation.authority_class` CHECK also uses the hyphen~~ **[RESOLVED.]** 34 §2.2 and 32 §4.3 both now use snake_case `accountable_autonomous`, `32`'s column renamed `agent_authority` | Reconcile on `accountable_autonomous` | Closed — no correction needed | **Resolved.** Nothing in this document depended on the outcome — this agent's value is `delegated` either way |
...
| 15 | **09 §4.4.2, with 01 §8.6** | ~~**No document specifies the network path or the identity model for an agent runtime's own LLM completion calls.**~~ **[RESOLVED.]** `09-monorepo-and-conventions.md` §4.4.2 now carries `agents/* → domino-platform namespace (AI Gateway / LLM Endpoint), config-gated`, resolved together with the adjacent `gateway → domino-platform`/`domino-compute` gap this row named, and states the credential-custody position this row asked for: *"a per-runtime External Secret to the serving path, not a single custodial service — each agent's own workload identity, not a shared one, per `40-copilot.md` §16 OQ-40-3"* | Closed — no correction needed | **Resolved.** OQ-40-3 is closed by the same amendment |
