...
|---|---|---|---|
| **A** | **`ToolManifest.target.slug` is typed `SubAppSlug` (10 §7.2), which contains only the nine sub-applications (10 §4.2). `knowledge-retrieval` and `reference-data` are `PlatformServiceSlug` values and are therefore unrepresentable as manifest targets** — yet 35 §8 marks `POST /retrievals` `x-agent-eligible: yes`, 35 §11.5 expects *"manifest tests for every manifest in `packages/agent-tooling/manifests/knowledge-retrieval/`"*, and 12 §3.1 marks fourteen `reference-data` operations agent-eligible | **YES — hard block.** Manifests 5 and 6 cannot be authored, and without manifest 5 the Copilot cannot cite a technical-manual passage, which is one third of 01 §8.1's function | §16 correction 6: introduce `ToolTargetSlug = SubAppSlug ∪ {knowledge-retrieval, reference-data}` in `packages/canonical-schemas`, retype `ManifestTarget.slug`, and widen 03 §8.2's *"[s]ub-application slug"* wording and 03 §3.1's *"manifest directories"* sentence to match. **`tool-server`, `auth`, `audit`, `gateway`, `sync`, and `notification` remain excluded**, which preserves 34 §8.1's existing guard (*"the manifest schema already makes a manifest targeting this service unrepresentable"*) as a property of the narrower union rather than an accident of the wider one |
| **B** | **22 (PdM) declares no manifests at all** — the string "manifest" does not appear in the document — although 03 §8.2 names three by name (`pdm-fleet-triage`, `pdm-equipment-deepdive`, `pdm-whatif`), 30 §5.3 uses one of them in a worked token, and 09 §8.5's Definition of Done requires *"[m]anifest tests pass for every manifest in `packages/agent-tooling/manifests/<slug>/`"* | **YES.** Manifest 3 has no home and no conformance test | §16 correction 7: add a *"Agent tool manifests"* section to 22 in the shape of 21 §9.5, shipping at least `pdm-equipment-deepdive.v1` |
| **C** | **24 (Scheduling) declares no manifests** — same absence, although 24 §9.4 explicitly reasons about the Work-Package Planner reaching Scheduling *"through tool-server proxied HTTP against the operations marked `x-agent-eligible` in §9.1"* | **YES.** Manifest 4 has no home | §16 correction 8: add the section, shipping `maintenance-history-lookup.v1` and the planner's own manifest |
| **D** | **12 (Reference Data) and 35 (Knowledge & Retrieval) declare no manifests by name.** 35 §11.5 refers to *"every manifest in `packages/agent-tooling/manifests/knowledge-retrieval/`"* without naming one | **YES**, downstream of A | §16 corrections 9 and 6 |
...
**The network path, which no document specifies.** **[FLAGGED — §16 correction 15.]** For the demonstration `LLMPort` is *"Domino AI Gateway fronting a hosted frontier model"* (01 §8.6). That is an egress from the agent runtime into a `domino-*` namespace, and 09 §4.4.2 sanctions no such edge for an agent runtime — its only Domino-adjacent rows are `domino-compute → gateway` and *"program ingress namespace → `domino-*` namespaces"*, and 30 §14 item 3 already flags that even `gateway → domino-platform` is unlisted. §13.3 declares the edge this runtime needs and §16 asks for the row.
...
|---|---|---|---|
| Egress | `kube-dns` | Service discovery | **Yes** — *"any service → `kube-dns`"*, if "any service" is read to include an agent runtime (§16 correction 18) |
| Egress | own CloudNativePG cluster `fathom-copilot-pg` | §8.2 | **Yes**, same reading |
| Egress | `auth` | JWKS; `POST /agent-runs`, `/checkpoint`, `/terminate` (31 §8) | **Yes**, same reading |
| Egress | `audit` | The `agent_answer` record (§16 correction 10) | **Yes**, same reading |
| Egress | **`tool-server`** | **Every tool call** (§4.6, 34 §3.1) | ⚠️ **NO ROW EXISTS** — §16 correction 19 |
| Egress | **`domino-platform` (AI Gateway / LLM Endpoint)** | `LLMPort` (§8.3) | ⚠️ **NO ROW EXISTS** — §16 correction 15 |
| Ingress | `gateway` **only** | §9.3 | ⚠️ **NO ROW EXISTS** — §16 correction 19 |
...
- **`agents/* → domino-platform`.** No precedent grants it, and the nearest neighbour is already flagged: 30 §14 item 3 records that *"[t]he table lists 'program ingress namespace → `domino-*` namespaces' but does not list `gateway` → `domino-platform` / `domino-compute`, which is required by 03 §8.3's proxied-Endpoint rule (D12)"*, and calls it **blocking deployment**. The Copilot's `LLMPort` egress is the same class of cross-plane edge and should be resolved with it, in one amendment rather than two. §16 correction 15 and **OQ-40-3** carry it, and the alternative (route completions through `gateway`) is recorded there rather than dismissed.
...
|---|---|---|---|---|
| 1 | **30 §5.3 vs 31 §3.2 vs 34 §4.5/§5.3** | **The delegated-token wire contract is specified three incompatible ways.** 31 §3.2: one token, `sub` = the human, `act` = the agent, nested `fathom.agent` block, `scope` carrying `sfx:` classes. 34 §4.5: *"[t]he caller's own delegated token, forwarded unchanged"*, with a header table containing exactly one `Authorization` row. 30 §5.3 hop 2: **two** credentials — `Authorization` = the agent's own workload token plus `X-Fathom-Delegation` — re-exchanged at the gateway, with flat `fathom:authority_class` / `fathom:manifest` claims and operation-shaped scope strings (`pdm.predictions.read`). Under 30's model the tool server would have to forward a delegation header it does not carry, and 34 gate 2's `delegated-authority-lapsed` check would inspect the workload token's expiry rather than the delegation's — the wrong token, in the gate built to catch exactly that | Reconcile to one contract. This document builds against **31 §3.2**, for the three reasons in §3.3. 30 §5.3's **audience-narrowing argument must survive** the reconciliation on its merits (a token audienced at six slugs is replayable across six), so the resolution is probably 31's claim shape with 30's per-target narrowing — but that is an architecture decision, not a build one | **Blocking integration for all three Wave-5 runtimes.** Not applied; flagged. This is **R7** and **G3** |
...
| 6 | **10 §7.2 `ManifestTarget.slug`, with 03 §8.2 and 03 §3.1** | **`slug` is typed `SubAppSlug`, which 10 §4.2 restricts to the nine sub-applications. `knowledge-retrieval` and `reference-data` are `PlatformServiceSlug` values and are therefore unrepresentable as manifest targets** — yet 35 §8 marks `POST /retrievals` `x-agent-eligible: yes`, 35 §11.5 expects *"manifest tests for every manifest in `packages/agent-tooling/manifests/knowledge-retrieval/`"*, and 12 §3.1 marks fourteen `reference-data` operations agent-eligible. 03 §3.1's sentence about *"manifest directories"* appears under a heading that reads *"Sub-application slugs"*, which is the root of it | Introduce `ToolTargetSlug = SubAppSlug ∪ {knowledge-retrieval, reference-data}` in `packages/canonical-schemas`, retype `ManifestTarget.slug`, and widen 03 §8.2's *"[s]ub-application slug"* wording. **`tool-server`, `auth`, `audit`, `gateway`, `sync`, and `notification` stay excluded**, which preserves 34 §8.1's existing guard as a property of the narrower union rather than an accident of the wider one | **BLOCKING.** Without it the Copilot cannot cite a technical-manual passage — one third of 01 §8.1's stated function. This is **R9**. Not applied; flagged |
| 7 | **22 (PdM)** | **Declares no agent tool manifests at all** — the string "manifest" does not appear in the document — although 03 §8.2 names three (`pdm-fleet-triage`, `pdm-equipment-deepdive`, `pdm-whatif`), 30 §5.3 uses one in a worked token, and 09 §8.5's Definition of Done requires *"[m]anifest tests pass for every manifest in `packages/agent-tooling/manifests/<slug>/`"* | Add an "Agent tool manifests" section in the shape of 21 §9.5, shipping at least `pdm-equipment-deepdive.v1` with the operation set of §4.2.3 and its conformance test inside PdM's own suite (03 §8.4) | **Blocking.** Not applied; flagged. §4.3 gap B |
| 8 | **24 (Scheduling)** | Same absence. 24 §9.4 reasons at length about the Work-Package Planner reaching Scheduling *"through tool-server proxied HTTP against the operations marked `x-agent-eligible` in §9.1"* and then ships no manifest for it to reach them through | Add the section, shipping `maintenance-history-lookup.v1` (§4.2.4) and the planner's own manifest | **Blocking.** Not applied; flagged. §4.3 gap C |
| 9 | **12 (Reference Data) and 35 (Knowledge & Retrieval)** | Neither names a manifest. 35 §11.5 refers to *"every manifest in `packages/agent-tooling/manifests/knowledge-retrieval/`"* without naming one; 12 marks fourteen operations agent-eligible and ships no manifest | Ship `knowledge-procedure-lookup.v1` (§4.2.5) and `reference-data-vocabulary-lookup.v1` (§4.2.6), each with its conformance test in the owning service's suite | **Blocking**, downstream of item 6. Not applied; flagged. §4.3 gap D |
...
| 18 | **09 §4.4.2** | The table's peer vocabulary is *"any service"*, *"the nine"*, and named platform slugs. 09 §4's own scoping restricts "service" to *"every one of the seventeen services"*, and an agent runtime is not among them — so it is not clear that `any service → auth`, `→ audit`, `→ kube-dns`, or `→ own database` covers an agent runtime at all. Every one of those four edges is needed (§13.3) | Either widen the four *"any service"* rows to *"any program workload"*, or add an explicit agent-runtime row set. Ambiguity here means the egress-equality assertion of 09 §8.6 is unfalsifiable for this workload class | **Blocking deployment.** Not applied; flagged |
| 19 | **09 §4.4.2** | **No row governs agent-runtime egress to `tool-server`, or gateway ingress to an agent runtime.** The table grants `tool-server → gateway` as *"yes, one rule, pass-through only"* — the *second* hop of a two-hop path whose first hop is unlisted | Add `agents/* → tool-server` (one rule, no agent holds an edge to any of the nine, to `knowledge-retrieval`, or to `gateway`) on the identical reasoning the table already gives for `tool-server → gateway`; and `gateway → agents/*` on 31 §4.1 step 5. **The absence of a direct `agents/* → <slug>` edge is the enforcement of §4.6 rule 1**, so granting the narrow row is what makes the prohibition testable | **BLOCKING deployment.** Not applied; flagged. **`42-redesign-case-builder.md` §2.1 asks for the same row independently — two of three Wave-5 runtimes converging is the signal that it is a table gap, not a per-agent request** |
