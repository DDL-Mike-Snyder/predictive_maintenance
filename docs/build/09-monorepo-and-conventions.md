...
| Secrets | **External Secrets Operator**. No secrets in charts, no secrets in `values.yaml` | 01 §11. **[amendment]** One documented exception: `apps/practitioner`'s workload client secret is injected by **Domino's** own project/environment credential mechanism, not External Secrets — a direct consequence of not deploying that pod ourselves (§9.5 item 28). This is an accreditation-relevant divergence, not an implementation detail: the credential sits outside the program's secret custody and rotation runbook, exactly as `31-auth.md` §5.7 already records for the Domino Endpoint's own static token (*"[r]otation is a program runbook item, because Domino provides no rotation policy"*). The same runbook note applies here; `52-practitioner-apps.md` §2.4/§13 correction 5 names the exception and assigns no rotation owner beyond recording that one is needed |
...
│       └── host/                     # Co-resident FastAPI process: serves the SPA bundle, proxies /api/
                                       #   to the gateway, holds the workload credential [02 §4.1's single-
                                       #   container rule]. A Python package; reached by the root
                                       #   pyproject.toml's shared ruff/mypy/pytest config like every
                                       #   other service. [amendment, 52-practitioner-apps.md §0.1]
│
...
| `apps/practitioner` | Domino-hosted practitioner surfaces | This document §2.6 + 02 §4.1 constraints; content `50-ui-design-system.md` and `52-practitioner-apps.md` [amendment] |
...
| `agents/<name>` | Prompt, manifest pin, **model pin** `[AMENDMENT — was previously omitted from this row despite §3.1's tree above already naming it]`, API version pin, evaluation set, deployment spec | 01 §8, 03 §8. Not this document |
...
| `agents/*` → `tool-server` | yes, **one rule**, per-agent binding enforced at the tool server, not the network layer | **[amendment 09-4]** — every tool call an agent makes, to any of the nine sub-applications or to `knowledge-retrieval`, is proxied through `tool-server` (`34-tool-server.md` §2.3), on the identical reasoning this table already gives for `tool-server → gateway` below. **The absence of a direct `agents/* → <slug>` edge is what makes 09 §9 item 15's prohibition enforceable at the network layer, not merely a coding convention** — flagged independently by `40-copilot.md` §16 correction 19 and `42-redesign-case-builder.md` §18 item 9(a); two of three Wave-5 runtimes converging on the same missing row is the signal that this is a table gap. This row covers a **Helm-deployed** agent runtime chart, pod-labelled `fathom.navy/service` like any of the seventeen services, running in `fathom-sustainment` |
| `domino-compute` namespace → `tool-server` | yes, **one rule** | **[AMENDMENT]** A Domino Job-hosted agent runtime — `41-pma-prescreener.md` §14.1's chosen vehicle for its enterprise run-initiator — is not a Helm-deployed `agents/*` chart and carries no `fathom.navy/service` label (09:664 already establishes this property for any Domino-deployed workload). The row above's per-agent binding enforcement at the tool server holds identically; only the network selector differs, a namespace selector rather than a pod-label one, the same shape as the `domino-compute → gateway` row below |
...
| `agents/*` → `domino-platform` namespace (AI Gateway / LLM Endpoint), **config-gated** | yes, **one rule**, LLM completion calls only | **[amendment 09-4]** — no document specified the network path for an agent runtime's own LLM completion calls (01 §8.6 names three serving paths; this table sanctioned none of them). Credential custody: a per-runtime External Secret to the serving path, not a single custodial service — each agent's own workload identity, not a shared one, per `40-copilot.md` §16 OQ-40-3 |
| `gateway` → `domino-platform` / `domino-compute` namespaces | yes, **one rule** | **[AMENDMENT]** The adjacent row above resolved the agent-runtime half of this gap; this half — `30-gateway.md` §11.3/§14 item 3's own long-standing flag — was not actually added in that pass despite that document's note claiming it would be. Required for two reasons stated there: the Domino Endpoint proxy (`30-gateway.md` §5.6) and invoking Domino-hosted agent runtimes (01 §8.7). Closes `30-gateway.md` §14 item 3 |
...
27. **Do not assume a Domino capability document 02 rules out** — Extensions outside Domino Cloud, platform-side drift detection, platform-side prompt or manifest governance, **air-gapped App or agent hosting** **[AMENDMENT — was "air-gapped agent hosting" alone; 02 §4.2 states agents use the same hosting infrastructure as Domino Apps, so D13's platform blocker binds both identically, and `52-practitioner-apps.md` — the one document that actually builds Domino Apps for the self-managed target — had recorded it as closed rather than open]**, or agent hosting beyond the per-project caps. Each has a named fallback in 01 §9; use it. *(**D26**; 01 §9)*
