...
| **sub-application → sub-application** | **NO** | 03 principle 2. This is the whole point of the policy |
...
| Agent authority | The principal carries an `authority_class` of `delegated` or `accountable-autonomous`; `accountable-autonomous` principals are rejected by the dependency on any `state-changing` operation | 03 §8.3 |
