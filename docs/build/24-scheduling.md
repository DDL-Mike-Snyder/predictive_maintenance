...
- **`lead_time_days` and `condition_code` are carried because the optimizer depends on them.** D6 notes "`lead time` is named as a hard constraint but exists in no Supply event or operation"; [03 §6](../architecture/03-integration-contracts.md) rev 2 adds `lead_time` and `condition_code` to `part_availability.changed` and `GET /lead-times` to Supply's required surface `[D24]`. If a substituting Supply implementation omits them, the constraint in §4.3 is unsatisfiable and the optimizer must refuse to solve rather than solve without it. Note the direction: `lead_time_days` is an **input to the solve**, held locally, and is never sent on the reservation — Supply does not take it. **[AMENDMENT]** Supply's `lead_time` is a composite, not a scalar (`26 §7.x`: `{order_and_ship_time_days, procurement_lead_time_days, basis, observed_n, as_of}`), and this document never previously stated which component `lead_time_days` reads — a 12-versus-96-day discrepancy on a hard scheduling constraint, left unresolved. **Interim, conservative reading:** `lead_time_days := procurement_lead_time_days` when the line requires new procurement (no on-hand or due-in stock at an acceptable condition code covers the quantity), else `order_and_ship_time_days`. The conservative direction is deliberate — using the shorter figure for a part that in fact needs procurement understates the constraint and can produce a plan that cannot execute (D6/D24's exact failure mode), where using the longer figure for an already-stocked part only over-constrains the solve toward a later, still-executable date. **Recorded as OQ-SCH-1, Phase 3 SME to confirm the branch condition** — the shape above is not asserted as Supply's own semantics, only as this optimizer's reading of them.
...
**[AMENDMENT — real security defect, found in adversarial review and closed here.]** This operation had no classification or ABAC discussion anywhere in this document, despite serving per-candidate detail over a shared, capacity-constrained solve — the same D13 shape found and fixed elsewhere this session, three ways at once:

1. **`slack`/`counterfactual` disclose an invisible candidate's magnitude.** `capacity(w, skill)` (§4.2) is shared across every candidate competing for one `Availability`'s skill class. A visible, *excluded* candidate's counterfactual — *"included if C1 capacity for skill_class MM increased by 14 man-hours"* — is exact only because the CP-SAT core includes every consuming candidate's man-hours, visible or not; a compartmented candidate's consumption changing shifts that number on a fully visible candidate's own disposition record with no compartment boundary crossed explicitly. **Fixed by field suppression, not re-solving per reader**: the optimizer solves once, over the true population, exactly as documented — reproducibility (§4.3) is preserved. At read time, `GET /work-packages/{id}/explanation` omits `slack` and `counterfactual` (never rounds or zeroes them) on any disposition whose CP-SAT infeasibility core includes a candidate outside the caller's clearance; `reason_code` (e.g. `capacity_exhausted`) is always served, since the category alone does not name a magnitude.
2. **`consequence_rank` is an ES-3 violation as specified.** `27-fleet-status.md` §3.6's ES-3 rule — *"the rank of a visible item changes when an invisible item is excluded"* — applies verbatim: a compartmented candidate's `expected_consequence` changing reorders the visible ranking. Unlike `slack`/`counterfactual`, `consequence_rank` is a derived ordering with no further internal consumer (the optimizer itself ranks candidates for the solve on `expected_consequence` alone, per `22-pdm.md` §7.5 rule 1 — `consequence_rank` on `CandidateDisposition` is an explanation-surface artifact only), so it is safe to **recompute per caller** at read time over the visible-candidate subset — the same per-viewer renormalization `27-fleet-status.md` §3.2 uses, since nothing downstream depends on it being frozen.
3. **`count(dispositions) == count(candidates_in_scope)` is a correct internal invariant and a forbidden external shape.** Asserted inside the persisting transaction (below), it must never be violated internally — that is what stops a silently dropped candidate. But `27-fleet-status.md` §3.6 rule 1 forbids exactly this shape on a *response*: *"a `restricted_contributors_present` boolean with a count — never a description, a system, or a magnitude."* `GET /work-packages/{id}/explanation` therefore returns only the dispositions the caller is cleared to see, with no total count, no `candidates_in_scope` size, and no residual/excluded-count field anywhere in the payload — the internal assertion protects correctness; the response protects against disclosing what it excluded.
...
| `fathom_maintenance_action_records_total{producer_node, capture_completeness}` | The label stream's actual volume, split by hull-versus-shore. A hull whose count is zero across a patrol is D8 recurring operationally. **[AMENDMENT — real security defect, found in adversarial review and closed here; same shape as `23-pma.md` §5.6's admission-control metrics.]** `producer_node` here is literally `edge:<asset_id>` — a specific hull — on a metric this document's own DoD (below) requires to be scraped and checked. Exposed at the same broad Prometheus audience `23-pma.md` §5.4's amendment was written to close off, this metric is the identical existence oracle: a hull's non-zero count *is* its existence. **The per-patrol DoD verification (below) is the one legitimate use of the true per-hull label** — it is a specific, narrow-audience test assertion during a known patrol window, not a standing broadly-scraped production signal — but the **production metric's `producer_node` label must be remapped through the same coarse, pre-declared cohort mapping** `23-pma.md` §5.6 now defines, never the raw `edge:<asset_id>`, before it reaches any dashboard or alert with an audience broader than the specific verification this DoD item performs |
