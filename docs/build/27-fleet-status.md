...
|---|---|---|
| `FailurePrediction.p_failure` (§4.6) | ~~`float = Field(ge=0.0, le=1.0, …)`~~ | **[DONE]** `float \| None = Field(default=None, ge=0.0, le=1.0, …)` |
| `_calibration_gate` validator (§4.6) | Rejects a sub-floor prediction that is not `class_estimate` | **[DONE]** Additionally requires `p_failure is None` when `calibration_population < CALIBRATION_POPULATION_FLOOR`, and `population_hazard_rate is not None` in that case |
...
**[AMENDMENT — OD-4 resolved.]** This amendment has landed in `10-shared-packages.md` §4.6/§4.7, both halves: the field is nullable and the gate validator rejects a non-null `p_failure` below the floor. `fleet-status`'s producer-contract-violation handling (treating a non-null `p_failure` below `calibration_population < 50` as a contract violation, `basis = 'unassessed'`, `fathom_contract_violations_total{producer="pdm"}`) now guards against a non-conforming producer rather than a schema the conforming one cannot help but violate — defense in depth, not the only line of defense it used to be.
...
| 3 | `configuration.baseline_changed` | **[AMENDMENT]** First resolves the changed set — exactly one of `changed_items` (inline) or `changed_items_ref` (fetched from object storage) is set on every event, per 20 §6.2; an initial baseline (~1,200 positions) and a bulk allowance import are always the ref form, never inline, and reading `changed_items` unconditionally silently treats a full-hull change as a no-op. Then: update `rm_configuration_tree`; advance `baseline_epoch`; **apply the antecedent rule (§9.4)**; retire contributors for items no longer in the baseline; mark surviving prediction contributors `stale` | Delete (item left baseline) + `configuration_invalidation` insert | Yes, asset → fleet. Also drives `item_left_baseline` clears (§6.5) |
...
| **OD-4** | ~~`packages/canonical-schemas` must make `FailurePrediction.p_failure` nullable and tighten `_calibration_gate` (§4.6).~~ **[CLOSED.]** `10-shared-packages.md` §7.1 now has `p_failure: float \| None`; OQ-10 is resolved there too | Every consumer of `FailurePrediction` | This service's producer-contract-violation handling (§4.6) is now dead code for the case it guarded — a non-null `p_failure` below the gate is no longer a valid *or* possible payload shape from a conforming producer, though the check remains as defense in depth against a non-conforming one | Resolved in [10](10-shared-packages.md) / `packages/canonical-schemas` |
...
| 15 | **Every open decision in §14 is either resolved and this document updated, or explicitly accepted as a demonstration-scope risk with a named owner.** **[AMENDMENT] OD-4 is closed**: `FailurePrediction.p_failure` is nullable in `packages/canonical-schemas`, and the contract this service consumes now matches [03 §7.1](../architecture/03-integration-contracts.md). **OD-7 is a program gate**: the primary effectiveness metric has no owner, and this document declines to become one by default | §14 |
