...
| 10 — Unified Adjudication Queue | `/adjudication`, `/adjudication/:proposalId` | **Partially.** List, filters, sort, claim yes; **`adjudicate` has no specified request body** and the `authority_class` filter enum omits `security_officer` (§15, §22 rows 15–16) |
...
### 5.5 The `SC` card's queue pre-filter cannot use `authority_class`
...
**`security_officer` is not an accepted value of that parameter.** [30 §4.5](30-gateway.md) declares `authority_class` as *"repeatable; `maintainer|planner|supply_officer|design_authority|fleet_authority`"* — five values, sourced to [03 §7.2.1], which has **six**. [30 §8.1.2] in the same document says the session returns *"its **six** `authority_classes`"*, so [30](30-gateway.md) contradicts itself. §22 row 15, **blocking for sheet 11's filters**.

**Interim, and it is a real filter rather than a workaround:** the `SC` card and §16's screen filter on **`kind`**, which [30 §4.5] declares as *"repeatable; `ProposalKind`"* and which [03 §7.2] extends with `purge | rewrap`. `?kind=purge&kind=rewrap` selects exactly the security officer's work, because [03 §7.2.1]'s table gives `security_officer` those two rows and only those two. The filter is narrower than `authority_class` would be — it cannot express *"a purge somebody else must counter-sign"* — and §16.4 handles that with `awaiting_second_signature`, which [30 §4.5] does accept.
...
**The `authority_class` enum omits `security_officer`, and the console cannot add it.** [30 §4.5] declares five values sourced to [03 §7.2.1], which has six; [30 §8.1.2] in the same document says the session returns *"its **six** `authority_classes`"*; [30 §14 item 8] says *"five classes"*. **[03 §7.2.1] is authoritative and has six**, `security_officer` added by amendment 03-1, and it owns the two `purge`/`rewrap` rows sheet 11 is built on. §22 row 15, **blocking**. Interim: the control lists the five values the parameter accepts, **states that `security_officer` is not selectable and why**, and points to `?kind=purge&kind=rewrap` (§5.5) as the equivalent selection. Sending an undeclared enum value would be a client sending an invalid request and would surface as a `422` the operator cannot act on.
...
**The queue row models two signatures and a class/fleet purge needs three.** [30 §4.5]'s `items[]` carries `adjudicated_by`, `second_adjudicator`, and the computed `second_signature_outstanding` — **and nothing that can hold a counter-signature distinct from the second signature.** [30 §2.4]'s read-model columns are the same two. So a `rewrap` at class scope requiring `security_officer` + a second `security_officer` + a `fleet_authority` counter-signature has three signatories and two fields. §22 row 19, **blocking for sheet 11's class and fleet rows.**
...
- [ ] Corrections **1–68** of §22 each filed against their document with an owner. **Rows 4, 15, 16, 17, 19, 21, 59, and 67 block a complete `apps/web`** and are recorded as blocking
...
| 15 | **30 §4.5 vs. 30 §8.1.2 vs. 03 §7.2.1** | `GET /proposals`'s `authority_class` parameter declares **five** values (`maintainer\|planner\|supply_officer\|design_authority\|fleet_authority`); [30 §8.1.2] in the same document says the session returns *"its **six** `authority_classes`"*; [30 §14 item 8] says *"five classes"*. [03 §7.2.1] has **six**, `security_officer` added by amendment 03-1, owning the two `purge`/`rewrap` rows sheet 11 is built on | Add `security_officer` to §4.5's enum and correct §14 item 8 | **Blocking** sheet 11's filters and the hub's `SC` pre-filter. Interim in §5.5 / §15.3 (`?kind=purge&kind=rewrap`). Flagged |
