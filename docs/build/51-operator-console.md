...
| **7** — no session identity operation, no logout, and [30 OQ-9] assumed a token the console does not hold | **Blocking the Persona Hub and the queue's default filter** | **CLOSED.** [30 §8.1.2](30-gateway.md) adds `GET /api/v1/gateway/session` (identity block, *"byte-identical to §3.2's token shape"*, with *"its six `authority_classes`"*) and `POST /api/v1/gateway/session/logout` (server-side RP-initiated logout, no client `end_session_endpoint` redirect). [30 §8.1.1](30-gateway.md) additionally adds the agent-invocation surface. [30 OQ-9](30-gateway.md) is now marked **[CORRECTED]** and reads *"The UI filters by `authority_class` using the roles from the session identity §8.1.2 returns — **not** 'its own token'"* | §4.3 and §5.2 wire real identity. **[50 §9.2]'s speculative field list is superseded**: the operation returns `fathom.identity`, whose subject field is **`subject_id`**, not `sub`, and which additionally carries `unit_path` and `qualifications` — §22 row 2. Cookie attributes and CSRF mechanism are now stated ([30 §8.1.2]: `HttpOnly`, `Secure`, `SameSite=Lax`, opaque identifier, backed by a table in the gateway's own CloudNativePG database (**[AMENDMENT]** not Redis — that would have contradicted [30 §6.3]'s own DECISION G-5 against a shared cache), double-submit token on state-changing gateway-owned operations), so [50 UI-OQ-1] is **partially** closed — **the cookie's name and the CSRF token's cookie and header names are still unspecified**, and the console cannot send a double-submit token whose name it does not know (§23 UI-OQ-1) |
...
| 00 — App Shell | layout route | **Mostly.** Identity and badge yes; the topbar lookup now has an operation — `GET /assets` declares query parameters (§4.4, §22 row 4, resolved) |
...
| 11 — Remediation & Purge Queue | `/audit/remediations`, `/audit/remediations/:proposalId` | **Partially.** Queue, panel, receipt yes; `Store` has no field (§16, §22 rows 17–18). **[AMENDMENT]** Row 19 (the queue row could not represent a three-signature purge) is resolved — `30-gateway.md` §2.4 added `counter_signature_by`/`counter_signature_at`. The dissemination ledger's missing read operation (row 67) is also resolved |
...
| `Readiness` | **`[GAP]`.** `GET /readiness`'s `scope` accepts `asset \| system \| fleet \| tycom` [27 §10.1 row 1, §2.2 — **[AMENDMENT]** was `fleet_grouping`/`grouping_id`, now `tycom`/`tycom_id`, closing 27's OD-2] — **`class` is not a scope value**. [27 §3.10] leaves TYCOM's intrinsic weight — *"hull tasking"* — an open figure, distinct from the now-closed scope-mechanism question | §22 row 8 |

**Resolution, `[ESTABLISHED HERE]`:** the box's label is WF's own `Rollup by TYCOM / class` and it renders **`scope=tycom`** rows — `GET /api/v1/fleet-status/readiness?scope=tycom` — with the `Class` column reading whatever TYCOM identifier the response carries in `subject_tycom_id` [27 §2.2]. **[AMENDMENT]** Previously `scope=fleet_grouping` / `subject_grouping_id` — 27-fleet-status.md has since adopted `tycom`/`subject_tycom_id` (10-shared-packages.md §4.5's `EventScope.TYCOM`), and this row is updated to match. **The console does not group by `class_id` and compute a per-class score**: a power mean over per-asset scores with an unspecified weight [27 §3.2, §3.10] is a rollup formula, and computing one in the browser would be manufacturing a readiness figure — the exact thing [04 §5]'s *"advisory overlay, not a readiness system of record"* decision forbids, and a client-side derivation [50 §11.3 item 20] besides. Where no `tycom` assessment exists, the box renders `EmptyState` naming the scope, and the `Hulls` column still renders from `asset_status` grouped by `class_id` **without a score column** — a class roster is a registry fact and is useful on its own.
...
**The sheet-note's `GET /readiness?scope=asset&id=` is not the operation.** [27 §10.1 row 1] declares `GET /readiness?scope=&asset_id=&system_id=&tycom_id=&view=&as_of=&changed_since=&limit=&cursor=` **[AMENDMENT — was `grouping_id`, closing 27's OD-2]** — the parameter is **`asset_id`**, not `id`. Trivial, and worth stating because a sheet-note is where an implementer looks first. §22 row 36.
...
**[27 §10.1] does not explicitly state that `scope=system` accepts `asset_id`** — the parameter list is flat (`scope=&asset_id=&system_id=&tycom_id=` **[AMENDMENT — was `grouping_id`]**) and the pairing is unstated. §22 row 39 asks for the clarification. The interim is the pairing above; the fallback, if it is rejected, is one call per system from `GET /assets/{asset_id}/systems`, which is a browser-side fan-out this document does not want and would file as a further correction.
...
| `target_sub_app` | Multi-select, repeatable | **[AMENDMENT]** The nine slugs [09 §7.1] plus the tenth, `audit` — a `purge`/`rewrap` proposal's `target_sub_app` is always `audit` [30 §7 table, `AUDIT_PROPOSAL_TOPIC`] — labelled with display abbreviations (§4.1's `src/display/`) |
...
**The signature state, from five fields plus one computed flag** **[AMENDMENT — was three fields; `counter_signature_by`/`counter_signature_at` added]** [30 §4.5]: `adjudicated_by`, `second_adjudicator`, `requires_dual_control`, `counter_signature_by`, `counter_signature_at`, `second_signature_outstanding`.
...
| **Request body** | **[AMENDMENT — resolved.]** `{decision: "approve" \| "reject", note}` [30 §4.6] — `note` required on `reject`, optional on `approve`. A verbatim pass-through to the owner's own adjudicate operation regardless of `kind` or which shape the owner exposes ([23 §3.7]'s `POST /proposals/{id}/adjudicate`; [32 §10.5]'s `PATCH /proposals/{id}`); the gateway does not invent a richer body. §22 row 17 closed |
...
**[AMENDMENT — §22 row 17 resolved, interim retired.]** The approve and reject buttons are no longer disabled: the request body is `{decision: "approve" | "reject", note}` (§4.6 above), sent for real. Approve carries no required `note`; reject requires one, and the button is disabled only for the ordinary reason of missing required input (an empty `note` on reject), never for an unspecified contract.
...
### 16.4 The `Signatures` column, and the three-signature purge **[AMENDMENT — the queue now represents it; heading retained for the row's history]**
...
| `blast_radius` `class` or `fleet` | **[AMENDMENT — resolved; row 19 is closed, not merely flagged.]** `security_officer · <n> of 2` plus, where `requires_counter_signature` is true, `fleet_authority · <✓ or ○>` sourced from the queue's own `counter_signature_by`/`counter_signature_at` columns (`30-gateway.md` §2.4) — no longer an inferred or unrepresentable state. The console **does not infer** the counter-signature's presence from `second_adjudicator`, because a second `security_officer` and a `fleet_authority` counter-signer are different people discharging different obligations and conflating them would report a two-signature purge as fully authorized |
...
39. ~~**Do not send a guessed adjudication request body.** The buttons are disabled with the reason stated until §22 row 17 lands.~~ **[AMENDMENT]** §22 row 17 has landed (`{decision, note}`, §4.6); this rule's outcome (never guess a field name) still applies to any *future* unspecified body, but the specific interim it described no longer holds. *(§15.7)*
...
- [ ] Adjudication: `ui-adjudicate-sends-if-match`, `ui-idempotency-key-stable-across-retries`, `ui-412-requires-reconfirm` green. **[AMENDMENT]** §22 row 17 is resolved — the buttons send the real `{decision, note}` body (§4.6); the disabled-with-reason fallback no longer applies
...
- [ ] Corrections **1–68** of §22 each filed against their document with an owner. **[AMENDMENT]** Row 4 is now resolved too — **only row 59 remains blocking**. **Rows 4, 15, 16, 17, 19, 21, and 67 are resolved.**
...
| 2 | **50 §9.2** | ~~Its asked-for session shape was `{ sub, display_name, unit_uic, billet, authority_classes[], clearance, classification }`. [30 §8.1.2] landed a different one...~~ **[RESOLVED.]** `50-ui-design-system.md` §9.2 now states the real member names (`subject_id`, `unit_path`, `qualifications[]`, no `classification`) | Closed — no correction needed | **Resolved.** §4.3 wires the real block, which now matches the spec it's wired against |
...
| 4 | **20 §6.1 rows 1–2** | ~~`GET /assets` and `GET /assets/{asset_id}` declare no query parameter anywhere~~ **[RESOLVED.]** `20-registry.md` §6.1 row 1 now declares `GET /assets?hull_or_tail=&uic=&domain=&class_id=&changed_since=&cursor=` — *"closes `51-operator-console.md` §22 row 4 (blocking)"* | Closed — no correction needed | **Resolved.** The sheet-00 lookup, sheet 02's search, and the hub's `VR` card all have an operation to call |
...
| 8 | **WF sheet 01; 27 §10.1 row 1** | `Rollup by TYCOM / class` needs a per-class readiness score. `scope` accepts `asset \| system \| fleet \| tycom` [**AMENDMENT** — was `fleet_grouping`, 27's OD-2 now closed by `10-shared-packages.md` §4.5] — **`class` is not a scope value**, and TYCOM's intrinsic weight (*"hull tasking"*) remains an open figure, separate from the scope mechanism | Either add a `class` scope, or state that a class is rendered without a rollup score (§6.6's interim already does the latter) | Interim in §6.6 (`tycom` rows; no client-side rollup). The scope-mechanism half of this row is resolved; the per-class score remains open |
...
| 16 | **30 §4.5, §4.3 rule 4** | ~~The queue row's **`subject` members are not enumerated.**~~ **[RESOLVED.]** `30-gateway.md` §4.5 now enumerates `subject`'s members per `scope` (the same `SCOPE_SUBJECT_FIELD` mapping events use), and names the provisional/confirmed pair explicitly: `subject.provisional_id`/`subject.confirmed_id` | Closed — no correction needed | **Resolved.** Sheet 10's target rendering, sheet 11's closure selector, and the `redesign_case` drill-down key can all be read directly, no generic key/value fallback needed |
| 17 | **30 §4.5, §4.6** | ~~**`POST /proposals/{proposal_id}/adjudicate` has no specified request body.**~~ **[RESOLVED.]** `30-gateway.md` §4.6: `{decision: "approve" | "reject", note}`, a verbatim pass-through to the owner's own adjudicate operation regardless of `kind` | Closed — no correction needed | **Resolved.** Sheets 07, 10, and 11's approve and reject actions are constructible; §15.7's disabled-buttons interim is no longer needed |
...
| 19 | **30 §4.5, §2.4; 03 §7.2.1; 32 §6.1** | ~~**The queue row cannot represent a three-signature purge.**~~ **[RESOLVED.]** `30-gateway.md` §2.4 added `counter_signature_by`/`counter_signature_at` to `PROJECTED_COLUMNS` and the wire row, distinct from `second_adjudicator`, plus `requires_counter_signature` and its own CHECK constraint | Closed — no correction needed | **Resolved.** §16.4 renders the third signature; no interim chip remains |
...
| 21 | **30 §3.2** | ~~**No fragment's `operation_id` value is stated.**~~ **[RESOLVED, with one residual note.]** `30-gateway.md` §3.2 now enumerates every fragment's real `operation_id` (09 §7.3's `<slug>_<verb>_<resource>` form, not a query string), and states explicitly where several fragments share one operation under different filters. **[VERIFY carried forward]**: `procedure_references`'s verb (`create` vs `search`/`retrieve` for a query-shaped `POST`) is not settled by 09 §7.3's single given example | Closed — no correction needed, verb choice aside | **Resolved.** Every composed-view field in §6–§11 is now derivable from the gateway's own contract |
...
| 39 | **27 §10.1 row 1** | The parameter list `scope=&asset_id=&system_id=&tycom_id=` **[AMENDMENT — was `grouping_id`]** is flat and **the valid pairings are unstated.** Sheet 01B's system rollup needs `scope=system` **with** `asset_id` to return all of one hull's system assessments in one call; the alternative is a per-system browser fan-out | State the valid `(scope, identifier)` pairings | Interim in §8.4 (the pairing above). Flagged |
...
|---|---|---|---|
| **UI-OQ-1** *(inherited, partly closed)* | **The session cookie's name and the CSRF token's cookie and header names.** [30 §8.1.2] now specifies the attributes and the mechanism — *"an opaque session identifier in an `HttpOnly`, `Secure`, `SameSite=Lax` cookie, keyed against a server-side store (**[AMENDMENT]** a table in the gateway's own CloudNativePG database, TTL-bound — not Redis, which would have contradicted [30 §6.3]'s DECISION G-5)"* and *"`SameSite=Lax` plus a **double-submit token** on every state-changing gateway-owned operation"* — **and names neither the cookie nor the token** | **The console cannot send a double-submit token whose header and cookie names it does not know**, so every state-changing gateway-owned request (claim, adjudicate, logout, agent-session) would be rejected by the middleware | `src/api/csrf.ts` exists as a stub reading a conventionally-named pair. **Raise for [30 §8.1.2] to name both.** The attributes half of [50 UI-OQ-1] is closed |
