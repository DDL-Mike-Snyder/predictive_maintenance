...
| *(absent from 03 §6 — mandated by 11 §8.4)* | **`fathom.registry.installed_item.identity_resolved`** | `fathom.registry.installed_item.v1` | `installed_item` |
...
`tools/check_event_catalog.py` must be taught this mapping rather than string-matching the catalog labels; §15 item 3 records the required document 03 §6 edit.
...
This exposes a gap in the library: **document 11 §2.3's `OutboxWriter.emit()` signature has no `partition_key` parameter**, while §2.2's outbox table has a `partition_key` column that is `NOT NULL`. For an `installed_item`-scoped event the partition key is not derivable from `subject`, because `subject` carries only `installed_item_id` [03 §5.4's "exactly one scope identifier"]. `emit()` must accept `partition_key: str | None = None`, defaulting to the scope identifier. Logged in §15 item 4.
...
| **2** | **03 §6** | **11 §8.4 mandates `installed_item.identity_resolved`** — *"Registry publishes `installed_item.identity_resolved`... on `fathom.registry.installed_item.v1`"* — and it appears in **no** row of 03 §6's Registry catalog. Without a catalog row it has no declared consumers, so no consumer-driven conformance test can be written for the only mechanism by which an event published under a provisional id remains interpretable | Add the row to 03 §6 with consumers `pdm`, `telemetry`, `supply`, `failure-intel`, `design-advisory` | **Published** regardless (§6.2, §7.3). `tools/check_event_catalog.py` will fail until 03 §6 is edited |
...
| **4** | **11 §2.3 vs §2.2** | `OutboxWriter.emit()` exposes no `partition_key` parameter, while the outbox table's `partition_key` column is `NOT NULL`. For an `installed_item`-scoped event the partition key is **not derivable from `subject`**, because §5.4 permits exactly one scope identifier and it is `installed_item_id` — but per-asset ordering requires `asset_id` (§7.1) | Add `partition_key: str | None = None` to `emit()`, defaulting to the scope identifier | **Blocking.** Registry cannot publish correctly-partitioned installed-item events without it |
...
| **11** | **10 §4.5** | `EventEnvelope` has **no `producer_node` field**, and `dedup_key` is the two-part `(producer.slug, monotonic_seq)` under OQ-5. **03 §5.4 now requires `producer_node`** and 11 §4.2 requires the three-part key | Add `producer_node` to `EventEnvelope`; make `dedup_key` three-part | Stale in 10. Registry's own events are unaffected in value (always `"enterprise"`) but the field must exist to be emitted |
