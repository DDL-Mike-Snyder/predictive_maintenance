...
| **Deferral** | Counts by `deferral_reason_class`. **Only `disagreement_with_risk` is evidence about prediction quality** — 03 §6 and D34: *"A deferral is a capacity or operational-tempo decision at least as often as a disagreement with the risk estimate; feeding it back as the latter biases models toward under-prediction."* Capacity, tempo, and parts-unavailability deferrals enter the propensity model as *opportunity* covariates, which is what they are, and enter no label as evidence |
...
| `criticality_tier.assigned` | `fathom.pdm.criticality_assessment.v1` | `niin` | `(niin, equipment_family)` | NIIN, `equipment_family`, tier, contributing factors, **transition annotation** (§8.4), `tier_policy_version`, `sme_validated`, `rescore_scoring_run_id` | `fleet-status`, `maintenance`, `audit` |
...
| `configuration.baseline_changed` | **Invalidation + epoch advance, in one transaction with the inbox record** (§8.1, `[D2]`). Also projects `policy_frozen` (§4.5.1) |
...
| `deferral.recorded` | `deferral_reason_class`. **Only `disagreement_with_risk` is evidence about prediction quality** (`[D34]`); the rest are opportunity covariates |
...
|---|---|---|---|---|
| **1** | **[10 §4.6](10-shared-packages.md)** `FailurePrediction` | `p_failure: float` is **required and non-nullable**, and OQ-10 records the contradiction with 06 §3 as unresolved, interim position *"`p_failure` required (03 is binding)."* **03 §7.1 has since been corrected**: `p_failure` is explicitly *"NULL when `calibration_population < 50`."* OQ-10's interim position is now the wrong one | `p_failure: float \| None = Field(default=None, …)`; `_calibration_gate` must additionally **require** `p_failure is None` when `calibration_population < CALIBRATION_POPULATION_FLOOR`, and require `population_hazard_rate is not None` and `reference_class is CLASS_ESTIMATE`. Add valid and invalid conformance vectors for the sub-gate case | **BLOCKING.** PdM cannot be conformant until applied. 10 §12 already names OQ-10 a Phase 3 blocker and a *"potential major schema change"*; it is now an actual one |
