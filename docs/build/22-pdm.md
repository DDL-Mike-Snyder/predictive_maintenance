...
CREATE TABLE pdm.label_observation (
    ...
    -- classification, inherited from the installed item [D13, union-label rule]. §6.1 AMENDMENT:
    -- the calibration cell this observation may contribute to
...
| CASREP history for the NIIN, fleet-wide | Scheduling read model of `maintenance_action.recorded` where `failure_indicator = true` and severity is CASREP-class | **IPCW-weighted** fleet failure count per operating-year per item | Mapped through a **fixed, pre-calibrated rate-to-score curve** — see below |
...
| Fleet-wide population of the NIIN | Registry configuration read model | Installed count | Mapped through a **fixed, pre-calibrated count-to-score curve**, inverted — see below |
...
**[AMENDMENT — real security defect, found in adversarial review and closed here.]** These two inputs were originally normalized by **live percentile rank within the domain** — where an item's score depended on the current failure-rate or population-count distribution of every other item, including items on compartmented hulls this caller may not be cleared to know exist. `27-fleet-status.md` §3.6 names this exact pattern and rules it out explicitly: *"Fleet-wide percentile normalization of an asset's score... The reference distribution is built over the full fleet contributor population, which includes compartmented contributors on other hulls. ES-2 fails across scopes rather than within one, which is harder to see and no less real."* A compartmented item's failure history on another hull was moving a visible item's criticality score, and therefore its `proposed_tier`, and therefore which model tier it received — with no compartment boundary crossed explicitly anywhere, and no test catching it, because `22-pdm.md` never mentioned "compartment" at all.

**Fixed instead, mirroring the other three inputs' own already-correct pattern** (mission-criticality and consequence-of-failure both use a *fixed* band map, `{0, 33, 67, 100}`, not a live rank): both inputs now map through a fixed, pre-calibrated curve set once by Phase 3 SME validation (the same `sme_validated`/`tier_policy_version` mechanism below already gates the whole formula) — a piecewise-linear or step function from IPCW-weighted failure rate (respectively installed count) to a `[0, 100]` score, with break points chosen from historical fleet-wide statistics **at calibration time**, never recomputed live against the current population. An item's score is then a pure function of its own rate/count and the fixed curve — it cannot move because a different item, compartmented or not, changed. `PLACEHOLDER — Phase 3 SME` for the curve's actual break points, same status as the weights below.

**[AMENDMENT — residual defect in the fix above, found in a follow-on adversarial review pass and closed here.]** The fixed curve closes the *normalization* leak — the curve's own break points no longer move — but leaves a second, distinct one standing: both raw inputs feeding that curve, *"CASREP history for the NIIN, fleet-wide"* and *"Fleet-wide population of the NIIN,"* are computed by pooling **every installed instance of that NIIN across the whole fleet**, exactly as `calibration_population` did before §6.1's fix. A NIIN's fixed-curve score is a pure function of its own rate/count, but that rate/count is not the *item's* own — it is shared identically by every physical item of that NIIN, compartmented or not. A compartmented hull's CASREP or a compartmented hull's installation/removal of that NIIN still moves the fleet-wide rate/count, and therefore still moves every *visible* item of that NIIN's criticality score and `proposed_tier`, with no compartment boundary crossed explicitly — the same shape 05 §6.1 already closed for `calibration_population`, one input over.

**Fixed the same way: both aggregates are compartment-scoped, mirroring §6.1's calibration cell.** The CASREP-history rate and the installed count are each computed over the subset of that NIIN's installed instances whose `compartments` is a subset of the scoring item's own — a baseline (uncompartmented) item's inputs draw only from the baseline pool, a compartment-`{X}` item's inputs draw from baseline ∪ compartment-`{X}`, and neither ever draws on a different compartment's instances. A visible item's criticality score is then a pure function of its own compartment-scoped rate/count and the fixed curve — it cannot move because a *different* item, in a different compartment, changed.
...
| **Method** | An ensemble of (a) the tier-2 degradation model, (b) a **physics-informed residual model** per family — the family's physical relation as a constraint, with the learned component modelling the residual — and (c) a **survival model conditioned on adjudicated causal features** from `causal_feature_set.updated`, with **every feature's own definition-time pinned** **[AMENDMENT — was "the set's declared `definition_time`"; `25-failure-intelligence.md`'s `causal_feature_entry` carries `definition_time` per entry, required and NOT NULL — there is no single set-level value to pin to]** to that feature's `definition_time`. Ensemble weights fitted on a weighted, time-split validation fold of the treated stratum. Uncertainty quantification by **weighted split-conformal prediction**, the covariate-shift weights being the §4.3 stabilized IPCW weights — this is the technically apt method precisely because censoring-induced shift is what the weights already describe, and it gives finite-sample interval coverage without assuming the degradation model is correctly specified |
| **Execution** | **Scheduled Domino Flow** for fleet scoring, per-mission-completion. **A dedicated Domino Endpoint for interactive what-if only** (01 §9), reached **exclusively through the gateway's Domino Endpoint proxy** (`POST /api/v1/gateway/domino/endpoint-invocations` with `{operation_id: "pdm_what_if"}` **[AMENDMENT — was a raw `domino_endpoint_name` path parameter; the gateway now resolves the actual endpoint from `operation_id` via this operation's own `x-domino-endpoint` manifest annotation (§5.3), never from a value this service supplies directly]**, `30-gateway.md` §5.6) **[AMENDMENT — this table and §13.1's egress row previously described a direct pdm-to-Endpoint call, which `31-auth.md` DO-NOT 13 forbids in terms: "Do not call a Domino Endpoint from a sub-application directly. It distributes a static, non-rotating, non-expiring credential across nine services... Route through the gateway proxy." `34-tool-server.md` §5.5 independently asserted the same prohibited direct-call topology as normative; both are corrected together, and `09 §4.4.2` now sanctions a `pdm → gateway` edge for exactly this]**. This service holds no Domino Endpoint credential of any kind. Endpoint constraints are real and bind the design regardless of which service holds the token: no autoscaling, 10 MB payload ceiling, ~60 s practical timeout, timed-out requests not cancelled, no serving SLO. `POST /what-if` therefore accepts **one item and at most 3 horizons per call**, enforces a **45 s** budget with a monotonic deadline, and returns 503 with `urn:fathom:problem:pdm:whatif-capacity` rather than queueing |
...
**[AMENDMENT — real security defect, found in adversarial review and closed here.]** `compartments` is on the key, and this was originally the pooled cell's most severe defect. The cell as first specified pooled **every** item-horizon observation in `(tier, equipment_family, horizon_days, reference_class, taxonomy_version, stratum)` regardless of the contributing item's own compartments, and `calibration_population` is the gate that flips `p_failure` from `NULL` to a value (§6.2). `27-fleet-status.md` §3.6's I2 doctrine applies verbatim, one level upstream of any rollup formula: *"A rollup whose value moves when a compartmented item degrades discloses that item's existence"* `[03 §7.3, D13]`. Here the disclosure is sharper than a moving score — it is a **discrete visibility flip**. A compartmented hull's item-horizon observation resolving as the cell's 50th pushes `p_failure` from absent to present, fleet-wide, for every *other*, fully uncleared item in that cell — and the reverse is equally true, since a compartmented item's data being *withdrawn* (re-accreditation, hull transfer) can push a cell back below the gate. No rounding or bucketing of the published count closes this, because the leak is the gate's own crossing, observable through `p_failure`'s presence regardless of what number is or isn't published beside it.

**Fixed by partitioning the cell, not by hiding the count.** `label_observation.compartments` (added below) carries the observation's own compartment set, inherited from its installed item exactly as 03 §7.3's union-label rule requires. A calibration record's `compartments` is the compartment set *of its own contributing pool*, and a record is only ever built from observations whose `compartments` is a **subset** of the record's own — never a superset. The baseline record (`compartments = '{}'`) is built exclusively from uncompartmented observations; a compartment-`{X}` record additionally pools baseline observations (permitted, since baseline is less classified than `X` and the union-label direction runs the same way it does everywhere else in this corpus) but never a different compartment `{Y}`'s data, and never leaks into the baseline record in the other direction. An item resolves its own gate and calibration **only** against the record whose `compartments` matches its own classification. This makes the exclusion exact and structural rather than a filter that a refactor can drop: a baseline item's `calibration_population`, gate state, and `p_failure` are a pure function of the baseline pool's SQL `WHERE compartments = '{}'`, and cannot move because a compartmented hull's observation was added, removed, or changed — mirroring `27-fleet-status.md` §3's ES-3 (*"adding, removing, or perturbing an invisible contributor cannot change $R(V)$ by any amount"*), enforced here at fit time rather than at read time. The accepted cost, symmetric with PMA's `scope_node` cohort restriction (`23-pma.md`), is that small compartments cold-start slower and fall back to a coarser `fallback_level` more often than they would in a pooled fleet-wide cell — a consequence of the compartment boundary being real, not a defect in the fit.

### 6.2 The gate — exactly as corrected

**`calibration_population ≥ 50` item-horizon observations in the cell** [FIXED — 06 §3, 03 §7.1], **evaluated against the caller's own compartment-scoped cell only** [AMENDMENT — §6.1].

`calibration_population` is the count of **resolved item-horizon observations** in the cell: item-horizon pairs whose horizon window has closed within the cell's window and whose outcome is determinate — a failure, or survival through the full horizon, or a dependent-censoring event carrying its IPCW weight. An open horizon is not a resolved observation. The count is **raw and unweighted**, exactly as the decision states, and is raw and unweighted **within the item's own compartment-scoped pool** (§6.1) — never the full cross-compartment fleet pool.
...
| Egress | `gateway` (Domino Endpoint proxy) **[AMENDMENT — was a direct Domino Endpoint egress; see the correction below]** | `POST /api/v1/pdm/what-if`'s only synchronous model call, proxied — never a direct sub-application-to-Domino edge |
