...
**[AMENDMENT — real security defect, found in adversarial review and closed here.]** The clearance predicate above closes the leak for the analyst *creating* the census — `:resolved_items` is resolved under **their** clearance before the count runs. It does nothing for a **different, later reader** of the same census, and `treatment_census` is deliberately immutable (`I3`, D21: a transparency surface that changed its own numbers after the fact would defeat the reason it exists). §8.1's `GET /hypotheses/{id}/treatment-census` and `POST /hypotheses/prior-examination-check` originally served this frozen record to every caller unconditionally. Since `causal_finding.published` is broadcast broadly — to `pdm`, `design-advisory`, `fleet-status`, `maintenance` (§9.1) — at a classification lower than "union of every compartment among the census's contributing items" would require, a reader cleared for the *finding* but not for one contributing hull's compartment could read `arms[].item_count` (or diff it against their own `POST /populations/preflight` result for the same fingerprint) and recover the compartmented contributor's existence and count — the exact channel `03 §7.3`'s D13 names, one layer beneath any rollup formula.

**Fixed by field suppression, not by re-deriving the census** — re-deriving a reader-scoped count would itself violate the immutability I3 requires, since D21's whole point is that a published verdict names the *exact* evidence it rests on, not a version reprojected per viewer. Instead, `treatment_census.classification` carries the true union (`inherited_from` every resolved item's own classification, per `03 §7.3`'s label rule), and both operations apply it as a field-level gate, mirroring `35-knowledge-retrieval.md`'s DO-NOT-14 suppression pattern rather than `27-fleet-status.md`'s renormalization (renormalization needs a re-derivable value; this one is architecturally frozen): a caller whose clearance dominates `classification` receives `arms[]`, `resolved_item_count`, and every derived count field in full; a caller who does not receive only the qualitative fields a verdict is reported with — `gate_verdict`, `finding_class`, `strength_band`, `treatment_handling`, `feedback_provenance` — with `arms[]`, `resolved_item_count`, `max_confounding_risk_fraction`, `differential_exposure`, and `worst_prediction_resolve_rate` omitted from the response entirely, never zeroed or rounded. An omitted field, not a suppressed number, is the honest signal here for the same reason `22-pdm.md` §6.3 gives it for `p_failure`: a rounded or zeroed count is still a number an uncleared reader can watch move.
...
| **`GET /hypotheses/{id}/treatment-census`** | **The D21 transparency surface.** The full per-arm census, gate verdict, propensity spec reference, balance diagnostics, and residual confounders behind this hypothesis, **count fields gated on the caller's clearance dominating `treatment_census.classification`** `[AMENDMENT — §2.8]` | `required` | `none` | yes |
...
| `POST /hypotheses/prior-examination-check` | §7.3. Fingerprint plus every prior examination, **each prior's count fields gated the same way as `treatment-census`** `[AMENDMENT — §2.8]` | `required` | `none` | yes |
...
| `installed_item.identity_resolved` **[AMENDMENT — this service is a declared 03 §6 consumer (04 §8) but never implemented the handler]** | `registry` | `resolution: superseded` — re-key `items` and any open attribution candidate from `provisional_id` to `canonical_id`; `confirmed` — no-op | `items` |
