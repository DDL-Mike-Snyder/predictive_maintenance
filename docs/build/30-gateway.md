...
|---|---|---|
| **Pass-through** | `/api/v1/{slug}/…` for the nine sub-applications plus `tool-server`, `knowledge-retrieval`, `notification`, `reference-data`, and three named `auth` operations: `POST /authority-checks`, `GET /principals/{sub}`, and `POST /practitioner-exchange` **[AMENDMENT]** | The upstream's own contract, proxied. The gateway adds authentication, rate limiting, correlation, and audit; it changes nothing else. **[amendment, closes `52-practitioner-apps.md` §13 correction 6, blocking]** `auth` was previously entirely absent from the pass-through set, so `31-auth.md` §8's two browser-facing advisory operations were unreachable from any browser client — `apps/web` included. Both remain non-authoritative (§8.1.2's identity operation is the one that matters for presentation logic); this addition only lets a client *ask* before it acts, per `31-auth.md` §8's own stated purpose. **[AMENDMENT]** `practitioner-exchange` (`31-auth.md` §5.8) is added for the same structural reason: the practitioner host reaches `gateway` over the sanctioned `domino-compute → gateway` edge (09 §4.4.2) and has no other route to `auth` at all — `09 §4.4.2` sanctions no `domino-compute → auth` edge, and none is needed, since `gateway → auth` is already part of the universal "any service → auth" grant that row already lists. The exchange is proxied exactly like any other pass-through operation; `gateway` does not interpret its body |
...
| `test_d32_no_domain_readmodel_other_than_the_queue` | **[AMENDMENT — corrected; `gateway_session` was missing from this list despite §8.1.2 adding the table.]** The migration head's table set equals exactly `{proposal_queue, inbox, idempotency_keys, queue_rebuild_watermark, agent_session, agent_turn, agent_run, gateway_session, alembic_version}` — four more than originally asserted, all gateway-own-orchestration bookkeeping (§8.1.1's agent tables, §8.1.2's session store), never a domain read model | 5 |
...
- **`gateway → domino-platform` / `domino-compute` egress**, for the Domino Endpoint proxy (§5.6) and for invoking Domino-hosted agent runtimes (01 §8.7). **[AMENDMENT — resolved.]** `09-monorepo-and-conventions.md` §4.4.2 now names this edge explicitly, closing §14 item 3.
...
3. **Do not build a read model of any other domain aggregate.** No asset cache, no prediction cache, no configuration mirror, no readiness snapshot. The migration head's table set is asserted in CI. **[AMENDMENT — clarified and corrected; `gateway_session` was omitted from this list despite being added in §8.1.2.]** This forbids a **domain** read model — a cache of a fact another sub-application owns. It does not forbid the gateway's own bookkeeping about its own orchestration activity: `idempotency_keys` and `queue_rebuild_watermark` were already exceptions of exactly this kind, and `agent_session`/`agent_turn`/`agent_run` (§8.1.1) and `gateway_session` (§8.1.2) are the same kind again — a table appearing in the asserted set for a reason stated here is not a regression; an *unstated* table is. *(**D32**; §9.3, §10.1)*
...
| 3 | **09 §4.4.2 sanctioned edge set** | ~~The table lists *"program ingress namespace → `domino-*` namespaces"* but does not list **`gateway` → `domino-platform` / `domino-compute`**~~ **[RESOLVED.]** `09-monorepo-and-conventions.md` §4.4.2 now names `gateway → domino-platform`/`domino-compute` explicitly, with the D12 rationale | Closed — no correction needed | **Resolved.** §11.3 |
