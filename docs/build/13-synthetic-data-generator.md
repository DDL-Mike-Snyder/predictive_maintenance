...
| **OPD-9** | **This generator does not produce an unstructured corpus.** "Corpus" throughout this document means the structured dataset (configuration, telemetry, maintenance, supply) — never IETMs, 3-M narratives, CASREP text, test reports, or ECPs as free text with applicability metadata, which is what `docs/build/35-knowledge-retrieval.md` exists to serve and what finding D14's adversarial golden sets need as source material | Knowledge & Retrieval has nothing to index; D14's injection-resistance testing has no adversarial passages to draw on | Not in scope here. Recorded as [document 05 §2.8, finding D38](../architecture/05-architecture-review-findings.md) | Program — a distinct content-generation problem, not an extension of this generator's techniques |
