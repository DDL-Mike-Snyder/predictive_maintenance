...
| **18** | [31 §6.4]'s authority matrix omits `purge`/`rewrap`; `security_officer` in no cell | **RESOLVED, both halves.** [31 §6.4] now carries both kinds with `counter_signature_class: fleet_authority` at class/fleet. **Also relevant here:** the same amendment pass annotated `redesign_case`'s **class** cell with dual control in both [31 §6.4] and [03 §7.2.1], closing [28 §16] correction 3. §7.7 is written against the corrected rule. **[AMENDMENT]** The residual *"§2.5 … still say five"* half of [50]'s row is also now closed — [31 §2.5]/§2.4 read six everywhere (`176`, `183`) |
...
| No package or source installation at container start | **[AMENDMENT — corrected.]** The launch file itself is honoured: it contains one `exec` and no package manager, and `host-launchfile-no-install` greps both launch files. **But this discharges program discipline, not the platform blocker `[D13]`, and the two are not the same claim.** `01 §9`/`02 §6.2`'s actual finding is that **the Domino application runtime itself** installs packages at container start — *"internal engineering describes as categorically incompatible with air gap, with no workaround"* — a **platform blocker, not program discipline** (`05-architecture-review-findings.md` **D26**: *"the Domino application runtime itself installs packages at pod start — a platform blocker presented as program discipline"*). This app is Domino-hosted (§2.1), so it is on exactly the path D13 names. This document's launch-file discipline is necessary and, per D13, **not sufficient** — the same reading `40-copilot.md` §16 item 4 and `41-pma-prescreener.md` §14.5 already give for the agent-hosting case, which binds identically here since 02 §4.2 states agents *"use the same hosting infrastructure as Domino Apps."* Recorded as its own open item, not closed by this row |
...
- **[AMENDMENT — resolved; both operations are now reachable.]** [31 §8]'s advisory operation lives on `auth`, and [30 §8.1]'s pass-through surface now names `auth`'s `POST /authority-checks` and `GET /principals/{sub}` explicitly alongside the nine sub-applications, `tool-server`, `knowledge-retrieval`, `notification`, and `reference-data` — closing §13 correction 6 below. This app may pre-mark or dim on that advice if a future revision chooses to; the interim design in this section (no dimming, server refusals only, every refusal rendered with its `reasons`) remains the stated posture regardless, since it was correct on its own terms and not merely a workaround for unreachability.
...
| **Claim state** | **[AMENDMENT — resolved.]** `claimed_by`/`claimed_until`/`second_signature_outstanding` now render in the `adjudication` object. See §13 correction 12 | [25 §5.2] requires a claim lease; [25 §2.9]'s DDL, [25 §8.1]'s `claimed=`/`awaiting_second_signature=` filters, and now the response itself all carry it. **A second adjudicator can see claim state directly** — no more *"claim state unknown"* marker; the list renders `claimed_by` name-resolved, or `unclaimed` where genuinely null |
...
| `withdraw` | **[AMENDMENT — resolved.]** `decision: withdraw` exists in the six-value CHECK (25 §2.9) and is now documented on the operation itself (25 §8.1). §13 correction 14 | [25 §7.1] makes `withdrawn` a state reached by *"[a]djudicator judgment, with the reason required"*, and `decision: withdraw` produces it. **The withdraw control is rendered**, mirroring `retire`'s |
...
| **[AMENDMENT — resolved.]** `awaiting_second_signature=` filter and field now exist. §13 correction 15 | [25 §8.1] now declares the `awaiting_second_signature` filter, mirroring [30 §4.5]'s for proposals, and the response carries `second_signature_outstanding` directly — no client-side derivation needed |
...
| `pui-claim-state-not-synthesized` **[AMENDMENT — the response now carries claim fields directly (§6.1's amendment); test retained, asserting the list renders the real `claimed_by`, never a client-guessed value]** | The list renders `claimed_by` as received, never re-derived or defaulted client-side |
...
44. ~~**Do not render "unclaimed" for a hypothesis.** No claim field exists; unknown is the honest rendering.~~ **[AMENDMENT — superseded.]** `claimed_by`/`claimed_until` now exist in the response (§13 correction 12, resolved); render `claimed_by` as received, `unclaimed` where it is genuinely null — the same rule every other claimable resource in this program follows. *(§6.1)*
...
Found while reconciling, following [09 §11] and [26 §13]'s convention: each is a **defect or gap in the cited document or in the approved wireframe**, not a decision of this one. **None is applied upstream here.** **[AMENDMENT]** Of the thirteen originally marked blocking (2, 5, 6, 7, 10, 12, 14, 15, 16, 17, 18, 21, and 22), eight are now resolved — **5, 6, 7, 12, 14, 15, 16, and 18** (see their own rows below, each with a back-reference to where the fix actually landed). **2, 10, 17, 21, and 22 remain open.** Correction 3 is **resolved, not by the fix it originally asked for** — see its row below.
...
| **5** | **09 §2.4, §4.5, §4.4.2** | ~~**The workload client secret for a Domino-hosted app cannot use the program's External Secrets path**... [09 §4.4.2] also justifies the `domino-compute → gateway` edge solely by "scoring Jobs write predictions"... the gateway's ingress allow-list is rendered from `fathom.navy/service` pod labels, which a Domino-deployed App does not carry~~ **[RESOLVED, all three parts.]** (a) 09 §2.4 now states the credential-custody exception explicitly with the rotation-runbook note (mirroring 31 §5.7's identical treatment of the Endpoint's static token). (b) 09 §4.4.2's edge row is widened to name practitioner Apps alongside scoring Jobs. (c) The NetworkPolicy template (09 §4.4.2) now renders `fromNamespaces` as a namespace selector, and `gateway`'s own values declare `fromNamespaces: [ingress-nginx, domino-compute]` — the exact namespace-selected, unlabelled-peer rule this row asked for | Closed — no correction needed | **Resolved.** No longer blocking |
| **6** | **30 §8.1** | ~~The pass-through surface covers *"the nine sub-applications plus `tool-server`, `knowledge-retrieval`, `notification`, `reference-data`"* — **not `auth`**. So [31 §8]'s `POST /authority-checks`... and `GET /principals/{sub}` are unreachable from any browser client~~ **[RESOLVED.]** `30-gateway.md` §8.1 now names both `auth` advisory operations in the pass-through set explicitly, and — closing a related gap found in the same review pass — `POST /practitioner-exchange` alongside them, which is this app's own credential-acquisition path (§4.4) | Closed — no correction needed | **Resolved.** Both operations, and the exchange itself, are reachable. §4.7's no-dimming interim remains the design choice on its own merits, not a workaround |
| **7** | **30 §3.2** | ~~`redesign_case_detail`'s fragments are `redesign_case`, `dossier`, `impact_snapshot`, `cost_estimate`, `causal_findings`. No gate-decision fragment~~ **[RESOLVED.]** `30-gateway.md` §3.2 added the `gate_decision` fragment — *"closes `52-practitioner-apps.md` §13 correction 7, blocking"* | Closed — no correction needed | **Resolved.** The cost box's gate region is a single-call sheet; §5.3/§7.5's second-call interim no longer applies |
...
| **12** | **25 §2.9, §8.1** | ~~[25 §5.2] requires a claim lease and returns `412` without one, but the `CausalHypothesis` wire model carries no `claimed_by`/`claimed_until` and [25 §8.1] declares no claim filter~~ **[RESOLVED.]** `claimed_by`/`claimed_until` now render in the response and `claimed=` filters `GET /hypotheses`, mirroring [30 §4.5] | Closed — no correction needed | **Resolved.** §6.1 renders `claimed_by` directly; no more "claim state unknown" interim |
...
| **14** | **25 §5.2, §7.1** | ~~`adjudication_record.decision` is `approve \| reject \| downgrade \| defer \| retire`. `adjudication_state` includes `withdrawn`... but no `decision` value produces it~~ **[RESOLVED.]** `decision` includes `withdraw`, which produces `adjudication_state: withdrawn` (25 §2.9, §8.1). The `reject`-to-`unsupported`-versus-`refuted` mapping remains unstated — a residual, narrower gap | Closed for the `withdraw` half — no correction needed there | **Resolved** for the withdraw control (§6.4 renders it). The `reject` mapping sub-issue is unresolved and tracked separately, not blocking |
| **15** | **25 §8.1** | ~~[25 §5.2] requires dual control to publish at S3/S4, but `GET /hypotheses` declares no `awaiting_second_signature` filter and the response carries no second-signature field~~ **[RESOLVED.]** `25-failure-intelligence.md` §8.1 now declares `awaiting_second_signature` and the response carries `second_signature_outstanding` | Closed — no correction needed | **Resolved.** No client-side link-only workaround needed |
| **16** | **25 §8.1** | ~~`POST /causal-feature-set-admissions` is declared with no request body~~ **[RESOLVED.]** `25-failure-intelligence.md` §8.1 now specifies `{feature_key, definition_ref, definition_version, definition_time, equipment_family, standing, review_due}` | Closed — no correction needed | **Resolved.** The admission form is buildable |
...
| **18** | **28 §9.1 / 30 §4.5** | ~~Nothing maps a `case_id` to its live `proposal_id`~~ **[RESOLVED.]** `28-design-advisory.md` §9.1 added `case_id` to `GET /proposals`'s filters | Closed — no correction needed | **Resolved.** Sheet 09 can find the live proposal without relying solely on the link-travel interim |
