...
| **7** | No session identity operation, no logout | **RESOLVED FOR `apps/web` ONLY.** [30 §8.1.2] now declares `GET /api/v1/gateway/session` and `POST /api/v1/gateway/session/logout`, and states the cookie and CSRF posture [31 §1.3] deferred. **Neither operation is usable by `apps/practitioner`**, which holds no gateway session cookie — §4.7 and §13 correction 3 |
| **10** | `apps/practitioner` has no client ID, no token path, no credential | **PARTIALLY RESOLVED.** [31 §5.8] now extends [31 §5.4]'s two-credential shape to an app→gateway hop — but §5.8 itself is *"recorded as an amendment ask rather than a settled mechanism."* §4 builds against it and §4.6 states what changes if it is revised |
...
4. **The credential model is entirely different.** The SPA holds no token — same as `apps/web` — but for a different reason and by a different mechanism: the co-resident host holds a **workload identity** and extends [31 §5.4]'s two-credential shape [50 §6.3, 31 §5.8]. §4.
...
| Workload credential and caller-authority header | [31 §5.4], [31 §5.8], [50 §6.3] | §4.2, §4.4 |
...
4. **No audit write.** [09 §4.4.2]'s sanctioned edge set gives `audit` an inbound edge from any service or agent runtime; it gives none to a Domino-hosted app, and the gateway already writes the audit record for every call it receives [30 §5.8]. A second, unreconciled record written by an app whose identity binding is itself the open question would be worse than no record. **The consequence is stated plainly in §4.7**: if the gateway does not honour the caller-authority header for audit, the audit record names the workload and not the human, which is [D12]'s defect one level up.
...
### 4.1 What is settled, what is an amendment ask, and what this document builds against

Three facts, in tension, and the implementer must hold all three:
...
3. **[31 §5.8] is an amendment ask, and says so.** It extends [31 §5.4]'s two-credential shape from an Endpoint-proxy call to an app→gateway hop, and closes with: *"[t]his is recorded as an amendment ask rather than a settled mechanism, because it repurposes a proxy built for one caller shape … for a different one … a reviewer confirming that is exactly the review this amendment asks for."*

**What this document builds against: [31 §5.8] exactly as written**, with one addition it leaves implicit and §4.3 makes explicit — *how* the app comes to hold a Domino-linked identity it can hand onward without asserting it. §4.6 states what changes if the ask is revised, and §4.7 states the interim posture without softening it.
...
| The host **forwards the verified assertion verbatim** as the caller-authority credential (§4.4) | This is what makes it *not* a claimed subject. [31 §13 item 15]: *"[d]o not trust a caller's assertion of who its end user is."* The gateway receives a signed assertion it can verify for itself, not a string the app asserts |
...
Implementing [31 §5.8]'s extension of [31 §5.4]:

```http
GET /api/v1/failure-intel/hypotheses?status=awaiting_adjudication&limit=50 HTTP/1.1
Host: <gateway>
Authorization: Bearer <the SURFACE's workload token>              ◀── who is calling  [31 §5.4]
X-Fathom-Caller-Authorization: Bearer <the verified Domino identity JWT, verbatim>
                                                                  ◀── on whose authority  [31 §5.8]
...
X-Fathom-Caller-Authorization: Bearer <verified Domino identity JWT>
...
Five properties, each of which a reviewer should be able to check in one place:

1. **Two credentials, both verified, neither synthesized.** The host verifies the inbound assertion (§4.3) and holds its own token (§4.2). It never mints, re-signs, or rewrites either.
2. **The header name is [31 §5.4]'s, unchanged.** `X-Fathom-Caller-Authorization`. Reusing the name is deliberate: [31 §5.8] extends that shape, and a second header name would make the two paths look like two mechanisms.
...
4. **Nothing else is added.** Explicitly not: any header a sub-application might authorize on [30 §8.4]. `host-adds-only-declared-headers` asserts the outbound header set is exactly the inbound set, minus the drops below, plus `Authorization`, `X-Fathom-Caller-Authorization`, and `X-Correlation-Id`.
5. **Dropped:** the inbound Domino identity header itself (replaced by the caller-authorization credential), any inbound `Authorization` (the browser has no business sending one), `Host`, and hop-by-hop headers.
...
### 4.6 If the amendment ask is revised

[31 §5.8] may be answered differently. Three alternatives, what each would mean, and precisely what changes here.

| Alternative | Assessment | What changes in this document |
|---|---|---|
| **(a) A token-exchange operation on `auth`** — RFC 8693, taking the verified Domino identity JWT and returning a short-lived `fathom` access token for that human | **The cleanest, and what this document would ask for.** It reuses [31 §4.2]'s exchange machinery, produces a token the gateway already validates unchanged [30 §5.1], removes the need for the gateway to understand a second credential shape, and makes the ABAC decision unambiguously about the human. It also gives the audit record the human's `sub` with no new mechanism | **One module.** `credential.py` gains the exchange call; the workload token is used only to authenticate *the exchange*, and the outbound call carries `Authorization: Bearer <exchanged user token>` with **no** `X-Fathom-Caller-Authorization`. §4.4's wire shape loses one header. **Nothing in the SPA changes**, because the SPA never sees a credential either way |
| **(b) Domino's extended identity-propagation tier** — [02 §4.1]: *"permitting an application to act with the user's full permissions subject to explicit user consent, default eight hours and up to thirty days"* | Gives the host a **Domino-side** credential acting as the user. It is still not a `fathom`-realm token, so the gateway must still broker or exchange — (a) by another route, with a consent prompt and a 30-day grant attached. It is also the *"only hard requirement"* for a future Extension [02 §4.1], so it may be enabled anyway | `identity.py` obtains a Domino credential instead of verifying a JWT; the outbound shape is unchanged; a consent-expiry path is added, rendering a stated reason rather than a bare `401` |
| **(c) A practitioner OIDC public client**, authorization-code + PKCE, in the iframe | **Rejected as a proposal, recorded so it is not re-proposed.** It puts a token in the browser, which [31 §4.1] and [50 §6.3] both forbid — *"a token in a browser is a token in every browser extension the browser has installed."* It also depends on storage for `state`/`nonce`, which is partitioned in a third-party iframe [50 §2.6 rule 3], and [02 §4.1]'s *"single deployment-wide subdomain"* makes the `redirect_uri` deployment-wide rather than per-App | Would invalidate §4.2–§4.5 entirely and require [50 §6.3]'s no-token rule to be amended first. Not built against |

**Whichever lands, two things do not change:** the SPA holds no credential, and the host adds nothing to a request that a sub-application might authorize on. Those are the invariants worth protecting through a revision.

### 4.7 The interim posture, stated without softening

[50 §6.3]: *"[u]ntil it lands, `apps/practitioner` is buildable as a read surface against operations its workload identity may reach, and its adjudication actions are blocked — which is the honest state, not a workaround."* [31 §5.8] repeats it: *"its state-changing adjudication actions … are blocked on this section landing."*

**One sharpening is required, and it is why this section exists.** "Buildable as a read surface" is only safe if the gateway evaluates authorization against the **human** the caller-authority credential names. If the gateway instead authorizes on the workload token alone and records the caller-authority credential for audit only, then every viewer of the App sees whatever the *workload identity* may see — a cross-viewer, potentially cross-clearance read, in a process that [02 §4.1] says shares resources across App users. That is not a degraded read surface; it is a leak.

**[ESTABLISHED HERE]** Three interim rules, and each is removable the moment [31 §5.8] is confirmed:

1. **The workload identity is provisioned at the narrowest possible envelope**: clearance `U` only, no compartments, no `cui_categories_authorized`, and **no `authority_classes`** [31 §2.3]. The consequence is exact and desirable: every read is level-scoped to `U`, and **every adjudication attempt is refused by the owning sub-application** with [31 §3.5]'s `urn:fathom:problem:auth:not-authorized` and its `reasons`. The write path is blocked *by authorization*, not by a disabled button — which is [50 §9.4]'s rule (*"[a] control that is disabled and unexplained is worse than one that fails with a reason"*) and gives an operator a legible failure.
2. **Both surfaces render a persistent, non-dismissible level-scoping notice** while the interim holds: *"This surface is showing content available at the U level to a shared application identity. Content above that level is excluded and is not reflected here."* It is a `SheetNote` in the annotation voice [50 §2.2's third rule], adjacent to the classification banner, and it is the same argument [06 §5] rule 3 and [27 §3.7] make for the contributor disclosure — *"[p]ublishing is therefore more protective than silence."* `pui-interim-scoping-notice` asserts it renders whenever `FATHOM_INTERIM_LEVEL_SCOPED` is set, and that it is outside every collapsed region.
3. **The README records the interim as a local resolution**, per [09 §8.7], naming [50 §13] correction 10, [50 §14] UI-OQ-4, [31 §5.8], and §14 P-OQ-3 — and states that the adjudication flows of §6.4 and §7.7 are **built and tested against a contract double** and are unreachable in a live deployment until the ask lands. They are built rather than deferred because the shape does not change under any of §4.6's alternatives.

**Two operations `apps/web` now has and this app does not.** [30 §8.1.2]'s `GET /api/v1/gateway/session` reads *"the session cookie's server-side session store"*, and this app holds no such session — so it cannot learn the viewer's `display_name`, `unit_uic`, `billet`, or `authority_classes`. `POST /api/v1/gateway/session/logout` is likewise inapplicable: there is no gateway session to end, and the Domino session is Domino's to end. **Consequences, accepted deliberately:**

- **No `IdentityBlock`.** Neither surface renders the viewer's name, organization, or authority chips. [50 §3.2] gives `apps/web` an `IdentityBlock` fed from [50 §9]'s session data; there is no equivalent data here, and inventing one from the Domino assertion's claims would be reading an identity the program does not own. §13 correction 3 asks [30 §8.1.2] for a caller-authority-borne equivalent.
- **No sign-out control.** [50 §6.3] made logout `apps/web`'s problem; here it is Domino's. A sign-out button that ended only our (nonexistent) session would be a lie about what it did. `pui-no-signout-control` asserts neither surface renders one.
...
| Component | Rule | Test |
|---|---|---|
| `CitationCard` | Renders `evidence_strength` **structured object** and `rendered_strength` **side by side**, with `posture`, `adjudication_state`, `confounders_unaddressed`, `treatment_assignment_handling`, `attribution_agreement`, and `evidence_strength_digest` | `pui-citation-carries-structured-strength` |
...
| **`evidence_strength_digest` is rendered** | It is [28 §8.2] property 1's whole enforcement mechanism, and a reviewer who can see the digest can ask whether it matches. Rendered as a short prefix with the full value available, alongside `evidence_strength_schema_version` and `rendered_by` — [28 §8.4]: *"names its renderer version so a rendering can be reproduced or repudiated"* |
...
| `ContributorDisclosure` | **No — but its logic recurs** | [27 §3.7]'s block is carried by readiness and explanation responses, which these surfaces do not call. **However**, §4.7's interim level-scoping notice is built on exactly [27 §3.7]'s reasoning: *"silent renormalization converts a partial view into something indistinguishable from a total view … Publishing is therefore more protective than silence."* And [30 §4.5]'s `queue_freshness.completeness: "level_scoped"` is the queue's own instance of the same rule and **is rendered** on Sheet 09 |
...
| `host-sends-two-credentials` | Every outbound call carries `Authorization` (workload) **and** `X-Fathom-Caller-Authorization` (verified assertion, verbatim) (§4.4) |
...
| `pui-interim-scoping-notice` | With `FATHOM_INTERIM_LEVEL_SCOPED` set, the notice renders outside every collapsed region (§4.7) |
...
| `pui-citation-carries-structured-strength` | The structured `evidence_strength` renders alongside `rendered_strength`, never instead of it; `confounders_unaddressed` renders unconditionally |
...
30. **Do not reason past [31 §5.8]'s status.** It is an amendment ask. Do not present the adjudication path as live, do not remove §4.7's level-scoping notice, and do not widen the workload identity's envelope to make a read work. *(31 §2.2's warning; 31 §5.8; 50 §13 correction 10; §4.7)*
...
- [ ] `host-sends-two-credentials`, `host-never-synthesizes-if-match`, `host-adds-only-declared-headers`, `host-passthrough-byte-identical`, `host-never-rewraps-problem`, `host-origin-required-on-unsafe` green *(§4.4, §4.5)*
- [ ] **The interim posture of §4.7 is implemented and recorded in the README, or [31 §5.8] has landed and the record says so.** The workload identity is provisioned at the narrowest envelope; `pui-interim-scoping-notice` and `pui-no-signout-control` green; [50 §13] correction 10, [50 §14] UI-OQ-4, and §14 P-OQ-3 all named *(§4.7)*
...
Found while reconciling, following [09 §11] and [26 §13]'s convention: each is a **defect or gap in the cited document or in the approved wireframe**, not a decision of this one. **None is applied upstream here.** The **fourteen** marked blocking prevent a complete `apps/practitioner`: **2, 3, 5, 6, 7, 10, 12, 14, 15, 16, 17, 18, 21, and 22**.
...
| **3** | **30 §8.1.2** | The amendment declares `GET /api/v1/gateway/session` reading *"the session cookie's server-side session store"* and `POST /api/v1/gateway/session/logout`. **Neither is usable by `apps/practitioner`**, which holds no gateway session — so this app cannot learn the viewer's `display_name`, `unit_uic`, `billet`, or `authority_classes` by any route. [50 §13] correction 7 is therefore resolved for `apps/web` and **still open for this app** | Add a caller-authority-borne variant — `GET /api/v1/gateway/session` resolving identity from `X-Fathom-Caller-Authorization` when no session cookie is present — returning the same identity block. It is the natural companion to [31 §5.8] and needs no new mechanism once that lands | **Blocking** the identity block and any authority-aware presentation. Interim in §4.7: no `IdentityBlock`, no sign-out, no pre-marking. Not applied; flagged |
...
| **P-OQ-3** | **[31 §5.8]'s confirmation** — does the gateway *authorize* against the caller-authority credential, or merely record it? | If it merely records it, the read surface leaks across viewers of different clearances (§4.7), not only the write path. This is the difference between a degraded surface and a defect | §4.7's three interim rules: narrowest workload envelope, a persistent level-scoping notice, and the README record. §4.6 names the preferred amendment (token exchange on `auth`) and what changes. **Raise with [31] and [30]** — this is [50 §14] UI-OQ-4, sharpened |
...
7. **[31 §2.2, §2.4, §5.4, §5.8, §6.4](31-auth.md)** — the federation direction, the six authority classes, the two-credential shape and its extension. **Read §5.8's closing paragraph before building anything on it.**
