...
Note for the shared-packages document: document 10 §4.5's `EventScope` enum has no `MISSION` member and its `SCOPE_SUBJECT_FIELD` map has no mission entry, while its own docstring observes that *"`mission_id` appears in `subject` but NO `scope` value selects it."* Document 03 §5.4 now settles it. Recorded as **OQ-4**.
...
**`installed_item.identity_resolved`.** Document 11 §8.4 introduces this event so consumers can resolve provisional edge-minted identities, and lists `telemetry` among the existing consumers of `installed_item.*`. **It does not appear in document 03 §6's Registry catalog rows.** This service therefore consumes only the two catalogued `installed_item.*` events and resolves aliases through `fathom-sync`'s `IdentityAliasResolver` (11 §8.4), which is fed by whatever the Registry publishes. Recorded as **OQ-5**: document 03 §6 needs the row, or document 11 §8.4 needs to name the catalogued event that carries the mapping. This service does not subscribe to an uncatalogued topic.
...
| **OQ-2** | **Document 04 §3 requires a mapping change to be "a versioned event"; document 03 §6's catalog has no such event** | Consumers learn of a mapping change only through `changed_since` and through `health_indicator.computed`'s bumped `channel_registry_version` | §3.1.6: version it, surface it via `changed_since` and the indicator payload, publish **no** uncatalogued event. Recommend adding `channel_mapping.version_published` to 03 §6 with consumers `pdm`, `pma` |
...
| **OQ-4** | **Document 10 §4.5's `EventScope` has no `MISSION` member**, while document 03 §5.4 requires `mission_id` when `scope=mission` and names `mission.completed` | `mission.completed` cannot be constructed from the shared schema package as published | Document 10 needs `EventScope.MISSION` and a `SCOPE_SUBJECT_FIELD` entry. Its own docstring already flags the gap as OQ-4 there |
| **OQ-5** | **`installed_item.identity_resolved`** (11 §8.4) is not in document 03 §6's Registry rows | Provisional-identity resolution has no catalogued event to consume | §8.3: consume only the catalogued `installed_item.*` events; resolve via `IdentityAliasResolver`. Document 03 §6 needs the row, or document 11 §8.4 needs to name the catalogued carrier |
